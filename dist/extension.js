...
</html>`}};function Z(){let l="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)l+=e.charAt(Math.floor(Math.random()*e.length));return l}var b=x(require("vscode"));var E=class{static providedCodeActionKinds=[b.CodeActionKind.QuickFix];projectDetector;fileNavigator;customDispatcher;constructor(e){this.projectDetector=e.projectDetector,this.fileNavigator=e.fileNavigator??new k,this.customDispatcher=e.dispatcher}provideCodeActions(e,t,r,n){let i=[];for(let o of r.diagnostics)if(o.source&&o.source.toLowerCase().includes("sonar")){let s=o.code?`\u26A1 Send to AI Agent (${o.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new b.CodeAction(s,b.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[o,e]},a.diagnostics=[o],a.isPreferred=!0,i.push(a)}return i}async executeFixWithAgent(e,t){let r=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),i;r.serverUrl&&n&&(i=new C({serverUrl:r.serverUrl,token:n}));let o=this.customDispatcher??new P({fileNavigator:this.fileNavigator,fetchRuleFn:i?g=>i.getEnrichedRule(g):void 0}),s=await o.getAvailableAgents(),c=b.workspace.getConfiguration("sonarAgent").get("defaultAgent");(!c||!s.some(g=>g.id===c))&&(c=s[0]?.id||"antigravity");let p="MAJOR";e.severity===b.DiagnosticSeverity.Error?p="CRITICAL":e.severity===b.DiagnosticSeverity.Information?p="MINOR":e.severity===b.DiagnosticSeverity.Hint&&(p="INFO");let v=b.workspace.asRelativePath?b.workspace.asRelativePath(t.uri):t.fileName,w={id:String(e.code||"sonar-issue"),ruleKey:String(e.code||""),message:e.message,component:v,filePath:v,line:e.range.start.line+1,severity:p,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()},h=await o.assemblePrompt(w);return o.dispatch(h,c,w)}};function ee(l){let e=u.initialize();l.subscriptions.push(e),u.info("Sonar Agent extension activated.");let t=y.workspace.workspaceFolders?.[0]?.uri.fsPath,r=new L({secretStorage:l.secrets,workspaceConfig:{get:(o,s)=>y.workspace.getConfiguration("sonarAgent").get(o,s),update:(o,s,a)=>y.workspace.getConfiguration("sonarAgent").update(o,s,a)},workspaceRoot:t}),n=new A(l.extensionUri,r);l.subscriptions.push(y.window.registerWebviewViewProvider(A.viewType,n)),l.subscriptions.push(y.commands.registerCommand("sonarAgent.refresh",async()=>{await n.refresh()})),l.subscriptions.push(y.commands.registerCommand("sonarAgent.configure",async()=>{await n.promptConfigureConnection()})),l.subscriptions.push(y.commands.registerCommand("sonarAgent.selectProject",async()=>{await n.promptProjectSelection()})),l.subscriptions.push(y.commands.registerCommand("sonarAgent.resetConnection",async()=>{await y.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await r.deleteToken(),await n.refresh(),y.window.showInformationMessage("SonarQube credentials have been removed."))})),l.subscriptions.push(y.commands.registerCommand("sonarAgent.showLogs",()=>{u.show()}));let i=new E({projectDetector:r});l.subscriptions.push(y.languages.registerCodeActionsProvider({scheme:"file"},i,{providedCodeActionKinds:E.providedCodeActionKinds})),l.subscriptions.push(y.commands.registerCommand("sonarAgent.fixWithAgent",async(o,s)=>{!o||!s||await i.executeFixWithAgent(o,s)}))}function te(){u.dispose()}0&&(module.exports={activate,deactivate});
