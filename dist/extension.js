...
`,t}async dispatch(e,t,n,i){try{await u.env.clipboard.writeText(e)}catch{}if(n&&n.line&&await this.fileNavigator.openFileAtLine(n.filePath,n.line),t==="copilot")try{return await this.executeCommandFn("workbench.action.chat.open",{query:e}),u.window.showInformationMessage("Dispatched Fix Prompt to GitHub Copilot Chat!"),{ok:!0,message:"Dispatched to GitHub Copilot Chat."}}catch{return u.window.showInformationMessage("Prompt copied to clipboard! Paste it into GitHub Copilot Chat."),{ok:!0,message:"Copied to clipboard (Copilot chat command not found)."}}if(t==="antigravity"){try{let s=[],a=i&&i.length>0?i:n?[n]:[],l=new Set;for(let p of a)if(p.filePath){let v=await this.fileNavigator.resolveFilePath(p.filePath);if(v&&!l.has(v)){l.add(v);let h=p.line&&p.line>0?p.line-1:0;s.push({uri:u.Uri.file(v),startLine:h,endLine:h})}}return await this.sendToAgentPanelFn({message:e,files:s.length>0?s:void 0,autoSend:!1}),u.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}for(let s of["workbench.action.chat.open","antigravity.prioritized.chat.open","workbench.action.openChat"])try{return await this.executeCommandFn(s,{query:e}),u.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}try{return await this.executeCommandFn("antigravity.openChatView"),u.window.showInformationMessage("Antigravity Chat opened & prompt copied to clipboard! Press Cmd+V / Ctrl+V to paste."),{ok:!0,message:"Chat opened and prompt ready in clipboard."}}catch{return u.window.showInformationMessage("Fix Prompt copied to clipboard for Antigravity Agent! Paste it into your agent chat."),{ok:!0,message:"Prompt ready in clipboard for Antigravity Agent."}}}let o=this.getAvailableAgents().find(s=>s.id===t),r=o?.name||(t==="claude-code"?"Claude Code":t==="cline"?"Cline":t==="roo-code"?"Roo Code":t==="continue"?"Continue":t==="codex"?"Codex Agent":"Clipboard");if(t==="claude-code"){let s=["workbench.view.extension.claude-sidebar","claude-code.focus","claude.focus"];for(let a of s)try{await this.executeCommandFn(a);break}catch{}}else if(t==="codex"){let s=["chatgpt.focus","openai.chatgpt.focus","codex.focus"];for(let a of s)try{await this.executeCommandFn(a);break}catch{}}else if(o?.focusCommand)try{await this.executeCommandFn(o.focusCommand)}catch{}return t==="clipboard"?(u.window.showInformationMessage("Fix Prompt copied to clipboard!"),{ok:!0,message:"Prompt ready in clipboard."}):(u.window.showInformationMessage(`Fix Prompt copied to clipboard for ${r}! Paste it into your agent chat.`),{ok:!0,message:`Prompt ready in clipboard for ${r}.`})}async resolveTargetAgent(e){let t=await this.getAvailableAgents();if(e&&t.some(i=>i.id===e))return e;if(e==="clipboard")return"clipboard";let n=this.getDefaultAgentFn();return n&&t.some(i=>i.id===n)?n:t[0]?.id||"clipboard"}async dispatchIssue(e,t){let n=await this.resolveTargetAgent(t?.targetAgentId),i=await this.assemblePrompt(e);return this.dispatch(i,n,e)}async dispatchBatch(e,t){if(!e||e.length===0)return{ok:!1,message:"No items to dispatch."};let n=await this.resolveTargetAgent(t?.targetAgentId),i=await this.assembleBatchPrompt(e);return this.dispatch(i,n,e[0],e)}async dispatchDiagnostic(e,t,n){let i="MAJOR";e.severity===u.DiagnosticSeverity.Error?i="CRITICAL":e.severity===u.DiagnosticSeverity.Information?i="MINOR":e.severity===u.DiagnosticSeverity.Hint&&(i="INFO");let o=u.workspace.asRelativePath?u.workspace.asRelativePath(t.uri):t.fileName,r={id:String(e.code||"sonar-issue"),ruleKey:String(e.code||""),message:e.message,component:o,filePath:o,line:e.range.start.line+1,severity:i,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()};return this.dispatchIssue(r,n)}};var H=b(require("vscode")),g=class{static channel;static initialize(e){return e?this.channel=e:this.channel||(this.channel=H.window.createOutputChannel("Sonar Agent",{log:!0})),this.channel}static sanitize(e){if(!e)return"";let t=e.replace(/https?:\/\/[^\s"'`<>]+/gi,"[SERVER]");return t=t.replace(/(token|bearer|authorization|password)\s*[:=]\s*[^\s,;&]+/gi,"$1: [REDACTED]"),t}static info(e){this.channel||this.initialize(),this.channel?.info(this.sanitize(e))}static warn(e){this.channel||this.initialize(),this.channel?.warn(this.sanitize(e))}static error(e,t){this.channel||this.initialize();let n="";t&&(t instanceof Error?n=` - ${t.message}`:n=` - ${String(t)}`),this.channel?.error(this.sanitize(`${e}${n}`))}static debug(e){this.channel||this.initialize(),this.channel?.debug(this.sanitize(e))}static show(){this.channel?.show(!0)}static dispose(){this.channel?.dispose(),this.channel=void 0}};var D=class{constructor(e,t,n,i,o){this.extensionUri=e;this.projectDetector=t;this.fileNavigator=n??new S,this.agentDispatcher=i??new A({fileNavigator:this.fileNavigator,projectDetector:this.projectDetector}),this.diagnosticCollection=o??c.languages.createDiagnosticCollection("SonarQube")}static viewType="sonarAgent.overviewView";_view;_webviewReady=!1;_lastStateMessage;fileNavigator;agentDispatcher;diagnosticCollection;resolveWebviewView(e,t,n){this._view=e,this._webviewReady=!1;let i=this.projectDetector.isConfiguredSync?.()??!1;g.info(`[Host] resolveWebviewView called. visible=${e.visible}, isConfigured=${i}`),e.webview.options={enableScripts:!0,localResourceRoots:[this.extensionUri]},e.webview.html=this._getHtmlForWebview(e.webview,i),e.webview.onDidReceiveMessage(async o=>{try{switch(g.info(`[Host] onDidReceiveMessage: command=${o.command}${o.text?` text="${o.text}"`:""}`),o.command){case"log":{g.info(`[Webview] ${o.text}`);break}case"ready":case"init":{this._webviewReady=!0,this._lastStateMessage&&this._view&&await this._view.webview.postMessage(this._lastStateMessage),await this._syncState();break}case"connect":{await this._handleConnect(o.serverUrl,o.token);break}case"disconnect":{await this._handleDisconnect();break}case"selectProject":{o.projectKey&&(await this.projectDetector.setProjectKey(o.projectKey),await this._syncState());break}case"switchProfile":{await this._handleSwitchProfile(o.profileId);break}case"createProfile":{await this.promptCreateProfile();break}case"openProjectPicker":{await this.promptProjectSelection();break}case"fetchDetails":{await this._handleFetchDetails(o.category);break}case"openFile":{await this.fileNavigator.openFileAtLine(o.filePath,o.line);break}case"sendToAgent":{await this._handleSendToAgent(o.item,o.targetAgentId);break}case"sendBatchToAgent":{await this._handleSendBatchToAgent(o.items,o.targetAgentId);break}case"setTargetAgent":{await c.workspace.getConfiguration("sonarAgent").update("defaultAgent",o.agentId,!0);break}case"refresh":{await this._syncState();break}}}catch(r){console.error("[SonarAgent] Webview message handling error:",r),this._view?.webview.postMessage({type:"error",message:r?.message||"Error processing request."})}}),this._syncState(),e.onDidChangeVisibility(()=>{e.visible&&this._syncState()})}async refresh(){this._view&&await this._syncState()}async promptProjectSelection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!e.serverUrl||!t){c.window.showWarningMessage("Please connect to SonarQube first.");return}let i=await new x({serverUrl:e.serverUrl,token:t}).fetchProjects(),o={label:"$(edit) Enter Project Key manually...",description:"Type the exact project key from SonarQube",detail:"Use this if your project is not listed or search is restricted"},r=[o,...i.map(a=>({label:a.name,description:a.key,detail:a.key===e.projectKey?"(Currently selected)":void 0}))],s=await c.window.showQuickPick(r,{placeHolder:"Select a SonarQube project or enter key manually",matchOnDescription:!0});if(s===o){let a=await c.window.showInputBox({prompt:"Enter the SonarQube Project Key",placeHolder:"e.g. org.company:my-project",value:e.projectKey||"",validateInput:l=>l.trim()?null:"Project Key cannot be empty"});if(a&&a.trim()){let l=a.trim();await this.projectDetector.setProjectKey(l),await this._syncState(),c.window.showInformationMessage(`Active SonarQube project set to: ${l}`)}return}s&&s.description&&(await this.projectDetector.setProjectKey(s.description),await this._syncState(),c.window.showInformationMessage(`Active SonarQube project set to: ${s.label}`))}async promptConfigureConnection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),n=!!(e.serverUrl&&t),i=[{label:"$(link) Update Server URL & Token",description:n?"Connected":"Not connected",detail:e.serverUrl?`Server: ${e.serverUrl}`:"Configure SonarQube host URL and authentication token",action:"updateCredentials"},{label:"$(folder-active) Select Sonar Project",description:e.projectKey?"Active":"Not selected",detail:e.projectKey?`Current project: ${e.projectKey}`:"Choose an active project on the server",action:"selectProject"},{label:"$(settings-gear) Open Extension Settings",detail:"Configure default AI agent and advanced preferences",action:"openSettings"},{label:"$(output) Show Extension Logs",detail:"Open the Sonar Agent output log channel",action:"showLogs"}];n&&i.push({label:"$(debug-disconnect) Disconnect & Reset Credentials",detail:"Remove stored token from OS Keychain and disconnect",action:"disconnect"}),this.profilesCapable()&&(i.splice(2,0,{label:"$(arrow-swap) Switch Connection Profile",detail:"Activate a different server + project + token binding",action:"switchProfile"}),i.splice(3,0,{label:"$(add) New Connection Profile...",detail:"Create a named server + project binding with live verification",action:"newProfile"}),i.splice(4,0,{label:"$(edit) Rename Connection Profile...",detail:"Rename a stored connection profile",action:"renameProfile"}),i.splice(5,0,{label:"$(trash) Delete Connection Profile...",detail:"Remove a profile and delete its token from the OS Keychain",action:"deleteProfile"}),i.splice(6,0,{label:"$(check) Verify Active Connection",detail:"Live-check the active profile against the SonarQube server",action:"verifyConnection"}));let o=await c.window.showQuickPick(i,{placeHolder:"Sonar Agent: Configure Connection & Settings",matchOnDescription:!0,matchOnDetail:!0});if(o)switch(o.action){case"updateCredentials":await this.promptUpdateCredentials(e.serverUrl);break;case"selectProject":await this.promptProjectSelection();break;case"switchProfile":case"newProfile":case"renameProfile":case"deleteProfile":case"verifyConnection":await this.runProfileAction(o.action);break;case"openSettings":await c.commands.executeCommand("workbench.action.openSettings","@ext:chulit.sonar-agent");break;case"showLogs":g.show();break;case"disconnect":await c.commands.executeCommand("sonarAgent.resetConnection");break}}async promptUpdateCredentials(e,t){let n=e||(await this.projectDetector.getConfig()).serverUrl||"http://localhost:9000",i=t||"";for(;;){let o=await c.window.showInputBox({title:"SonarQube Connection (1/2)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:n,ignoreFocusOut:!0,validateInput:l=>this.validateServerUrlInput(l)});if(o===void 0)return;n=o.trim();let r=await c.window.showInputBox({title:"SonarQube Connection (2/2)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:i,password:!0,ignoreFocusOut:!0,validateInput:l=>l.trim()?null:"User Token is required"});if(r===void 0)return;i=r.trim();let s={ok:!1};if(await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{s=await new x({serverUrl:n,token:i}).verifyConnection()}),!s.ok){if(await c.window.showErrorMessage(`SonarQube connection verification failed: ${s.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}await this.projectDetector.setServerUrl(n),await this.projectDetector.setToken(i),c.window.showInformationMessage("SonarQube connection successfully verified and saved!"),(await this.projectDetector.getConfig()).projectKey||await this.promptProjectSelection(),await this.refresh();return}}validateServerUrlInput(e){let t=e.trim();if(!t)return"Server URL is required";if(!t.startsWith("http://")&&!t.startsWith("https://"))return"Server URL must start with http:// or https://";try{if(!new URL(t).hostname)return"Please enter a valid URL with hostname"}catch{return"Please enter a valid URL"}return null}async promptManageProfiles(){let e=[{label:"$(arrow-swap) Switch Profile",action:"switchProfile"},{label:"$(add) New Profile...",action:"newProfile"},{label:"$(edit) Rename Profile...",action:"renameProfile"},{label:"$(trash) Delete Profile...",action:"deleteProfile"},{label:"$(check) Verify Active Connection",action:"verifyConnection"}],t=await c.window.showQuickPick(e,{placeHolder:"Sonar Agent: Manage Connection Profiles"});t&&await this.runProfileAction(t.action)}async runProfileAction(e){switch(e){case"switchProfile":{let t=await this.pickProfile("Select the connection profile to activate");if(!t)return;try{await this.projectDetector.activateProfile(t.id)}catch(n){c.window.showErrorMessage(n?.message||`Unknown connection profile: ${t.id}`);return}c.window.showInformationMessage(`Active connection profile: ${t.name}`),await this.refresh();break}case"newProfile":await this.promptCreateProfile();break;case"renameProfile":{let t=await this.pickProfile("Select the connection profile to rename");if(!t)return;let n=await c.window.showInputBox({prompt:`New name for profile "${t.name}"`,value:t.name,ignoreFocusOut:!0,validateInput:i=>i.trim()?null:"Profile name is required"});if(n===void 0||!n.trim())return;await this.projectDetector.renameProfile(t.id,n.trim()),await this.refresh();break}case"deleteProfile":{let t=await this.pickProfile("Select the connection profile to delete");if(!t||await c.window.showWarningMessage(`Delete connection profile "${t.name}" and its stored token?`,{modal:!0},"Delete")!=="Delete")return;await this.projectDetector.deleteProfile(t.id),c.window.showInformationMessage(`Connection profile "${t.name}" deleted. Create a profile to reconnect.`),await this.refresh();break}case"verifyConnection":{let t=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken();if(!t.serverUrl||!n){c.window.showWarningMessage("No active connection profile to verify.");return}let i={ok:!1};await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{i=await new x({serverUrl:t.serverUrl,token:n}).verifyConnection()}),i.ok?c.window.showInformationMessage("SonarQube connection verified."):c.window.showErrorMessage(`SonarQube connection verification failed: ${i.message||"Unknown error"}`);break}}}async pickProfile(e){let t=await this.projectDetector.listProfiles();if(t.length===0){c.window.showInformationMessage("No connection profiles yet. Create one first.");return}let n=await c.window.showQuickPick(t.map(i=>({label:i.name,description:i.id,detail:`${i.serverUrl} \xB7 ${i.projectKey}`})),{placeHolder:e,matchOnDescription:!0,matchOnDetail:!0});return n?t.find(i=>i.id===n.description):void 0}async promptCreateProfile(){let e=await this.projectDetector.getCreationSuggestion(),t="",n=e?.serverUrl??"http://localhost:9000",i="",o=e?.projectKey??"";for(;;){let r=await c.window.showInputBox({title:"New Connection Profile (1/4)",prompt:"Name this profile (e.g. kantor-prod)",value:t,ignoreFocusOut:!0,validateInput:h=>h.trim()?null:"Profile name is required"});if(r===void 0)return;t=r.trim();let s=await c.window.showInputBox({title:"New Connection Profile (2/4)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:n,ignoreFocusOut:!0,validateInput:h=>this.validateServerUrlInput(h)});if(s===void 0)return;n=s.trim();let a=await c.window.showInputBox({title:"New Connection Profile (3/4)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:i,password:!0,ignoreFocusOut:!0,validateInput:h=>h.trim()?null:"User Token is required"});if(a===void 0)return;i=a.trim();let l=await c.window.showInputBox({title:"New Connection Profile (4/4)",prompt:"Enter the SonarQube Project Key (optional, pick later)",value:o,ignoreFocusOut:!0});if(l===void 0)return;o=l.trim();let p={ok:!1};if(await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{p=await new x({serverUrl:n,token:i}).verifyConnection()}),!p.ok){if(await c.window.showErrorMessage(`SonarQube connection verification failed: ${p.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}let v=await this.projectDetector.createProfile({name:t,serverUrl:n,projectKey:o,token:i});e?.hasPlaintextCredentials&&c.window.showWarningMessage("sonar-project.properties contains plaintext credentials. They were not imported; remove them to avoid leaking secrets."),c.window.showInformationMessage(`Connection profile "${v.name}" created and activated.`),v.projectKey||await this.promptProjectSelection(),await this.refresh();return}}async _handleSendToAgent(e,t){await this.agentDispatcher.dispatchIssue(e,{targetAgentId:t})}async _handleSendBatchToAgent(e,t){await this.agentDispatcher.dispatchBatch(e,{targetAgentId:t})}async _handleFetchDetails(e){if(!this._view)return;let t=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken();if(!(!t.serverUrl||!t.projectKey||!n)){this._view.webview.postMessage({type:"loadingDetails",loading:!0});try{let i=new x({serverUrl:t.serverUrl,token:n}),o=[];e==="hotspots"?o=await i.getHotspots(t.projectKey):e==="coverage"?o=await i.getCoverageFiles(t.projectKey):e==="duplications"?o=await i.getDuplicationFiles(t.projectKey):e==="reliability"||e==="security"||e==="maintainability"||e==="accepted"?o=await i.getIssues(t.projectKey,e):o=await i.getIssues(t.projectKey),this._view.webview.postMessage({type:"details",category:e,items:o}),await this.syncDiagnostics(o)}catch(i){this._view.webview.postMessage({type:"detailsError",message:i.message||"Failed to load issues."})}finally{this._view.webview.postMessage({type:"loadingDetails",loading:!1})}}}async syncDiagnostics(e){this.diagnosticCollection.clear();let t=new Map;for(let n of e){if(!n.filePath)continue;let i=await this.fileNavigator.resolveFilePath(n.filePath);if(!i)continue;let o=c.Uri.file(i),r=n.line&&n.line>0?n.line-1:0,s=new c.Range(r,0,r,100),a=c.DiagnosticSeverity.Information;n.severity==="BLOCKER"||n.severity==="CRITICAL"?a=c.DiagnosticSeverity.Error:n.severity==="MAJOR"&&(a=c.DiagnosticSeverity.Warning);let l=new c.Diagnostic(s,n.message,a);l.code=n.ruleKey,l.source="SonarQube";let p=t.get(o.toString())||{uri:o,diagnostics:[]};p.diagnostics.push(l),t.set(o.toString(),p)}for(let{uri:n,diagnostics:i}of t.values())this.diagnosticCollection.set(n,i)}async _handleSwitchProfile(e){try{await this.projectDetector.activateProfile(e)}catch(t){this._view?.webview.postMessage({type:"error",message:t?.message||`Unknown connection profile: ${e}`});return}await this._syncState()}profilesCapable(){return typeof this.projectDetector.listProfiles=="function"}async _syncState(){if(this._view)try{let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),n=c.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),i=this.agentDispatcher.getAvailableAgents(),o=n;i.some(a=>a.id===o)||(o=i[0]?.id||"clipboard");let r=await this.projectDetector.listProfiles?.()??[],s=await this.projectDetector.getActiveProfile?.()??null;if(g.info(`[Host] _syncState: serverUrl=${e.serverUrl}, hasToken=${e.hasToken}, tokenPresent=${!!t}`),r.length===0&&!e.serverUrl&&!t){let a={type:"state",state:"no-profiles",serverUrl:"http://localhost:9000",profiles:r,activeProfileId:s?.id,defaultAgent:o,availableAgents:i};this._lastStateMessage=a,await this._view.webview.postMessage(a);return}if(e.serverUrl&&e.hasToken&&t){let a={type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:e.projectKey,projects:[],profiles:r,activeProfileId:s?.id,defaultAgent:o,availableAgents:i};this._lastStateMessage=a;let l=await this._view.webview.postMessage(a);g.info(`[Host] Immediate postMessage(connected) delivered=${l}`),await this._view.webview.postMessage({type:"loading",loading:!0});let p=new x({serverUrl:e.serverUrl,token:t}),v=e.projectKey,[h,P]=await Promise.allSettled([p.fetchProjects(),v?p.getOverview(v):Promise.resolve(null)]),k=h.status==="fulfilled"?h.value:[];h.status==="rejected"&&console.error("[SonarAgent] fetchProjects error:",h.reason);let y=v;!y&&k.length>0&&(y=k[0].key,await this.projectDetector.setProjectKey(y));let w=null,j;if(y&&y===v)P.status==="fulfilled"?(w=P.value,w&&g.info(`Measures updated for [${y}]: ${w.security.count} vulnerabilities, ${w.reliability.count} bugs, ${w.maintainability.count} smells, ${w.coverage.percentage.toFixed(1)}% coverage.`)):(j=P.reason?.message||"Failed to fetch project measures.",g.error(`Failed to fetch project measures for [${y}]`,j));else if(y)try{w=await p.getOverview(y),w&&g.info(`Measures updated for [${y}]: ${w.security.count} vulnerabilities, ${w.reliability.count} bugs, ${w.maintainability.count} smells, ${w.coverage.percentage.toFixed(1)}% coverage.`)}catch(Q){j=Q.message||"Failed to fetch project measures.",g.error(`Failed to fetch project measures for [${y}]`,j)}let R={type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:y,projects:k,overview:w,overviewError:j,profiles:r,activeProfileId:s?.id,defaultAgent:o,availableAgents:i};this._lastStateMessage=R;let V=await this._view.webview.postMessage(R);g.info(`[Host] Full postMessage(connected) delivered=${V}`),await this._view.webview.postMessage({type:"loading",loading:!1}),!this._webviewReady&&this._view&&setTimeout(async()=>{!this._webviewReady&&this._view&&this._lastStateMessage&&await this._view.webview.postMessage(this._lastStateMessage)},350)}else{let a={type:"state",state:"onboarding",serverUrl:e.serverUrl||"http://localhost:9000",profiles:r,activeProfileId:s?.id,defaultAgent:o,availableAgents:i};this._lastStateMessage=a;let l=await this._view.webview.postMessage(a);g.info(`[Host] postMessage(onboarding) delivered=${l}`)}}catch(e){console.error("[SonarAgent] _syncState error:",e),g.error("Failed to synchronize Sonar Agent state",e),this._view?.webview.postMessage({type:"error",message:e.message||"Failed to synchronize Sonar Agent state."}),this._view?.webview.postMessage({type:"loading",loading:!1})}}async _handleConnect(e,t){let n=(t||"").trim(),i=(e||"").trim();if(!i||!n){this._view?.webview.postMessage({type:"error",message:"Server URL and User Token are required."});return}!i.startsWith("http://")&&!i.startsWith("https://")&&(i="http://"+i),i=i.replace(/\/+$/,""),this._view?.webview.postMessage({type:"connecting"}),g.info("Verifying SonarQube credentials...");try{let r=await new x({serverUrl:i,token:n}).verifyConnection();if(!r.ok){g.warn(`Connection verification failed: ${r.message||"Unknown error"}`),this._view?.webview.postMessage({type:"error",message:r.message||"Connection verification failed."});return}await this.projectDetector.setServerUrl(i),await this.projectDetector.setToken(n),g.info("SonarQube connection verified and saved."),c.window.showInformationMessage("SonarQube connection successfully verified!"),await this._syncState()}catch(o){console.error("[SonarAgent] _handleConnect error:",o),g.error("Unexpected connection error occurred",o),this._view?.webview.postMessage({type:"error",message:o?.message||"Unexpected connection error occurred."})}}async _handleDisconnect(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!!!(e.serverUrl&&t)){this._view?.webview.postMessage({type:"disconnected",message:"Connection credentials cleared."}),c.window.showInformationMessage("Sonar Agent connection inputs cleared.");return}await c.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await this.projectDetector.deleteToken(),g.info("SonarQube credentials removed and disconnected."),this._view?.webview.postMessage({type:"disconnected",message:"Disconnected from SonarQube. Credentials removed."}),c.window.showInformationMessage("SonarQube credentials have been removed."),await this._syncState())}_getHtmlForWebview(e,t=!1){let n=te();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};function te(){let d="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)d+=e.charAt(Math.floor(Math.random()*e.length));return d}var E=b(require("vscode"));var L=class{static providedCodeActionKinds=[E.CodeActionKind.QuickFix];dispatcher;constructor(e){this.dispatcher=e.dispatcher??new A({projectDetector:e.projectDetector,fileNavigator:e.fileNavigator})}provideCodeActions(e,t,n,i){let o=[];for(let r of n.diagnostics)if(r.source&&r.source.toLowerCase().includes("sonar")){let s=r.code?`\u26A1 Send to AI Agent (${r.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new E.CodeAction(s,E.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},a.diagnostics=[r],a.isPreferred=!0,o.push(a)}return o}async executeFixWithAgent(e,t){return this.dispatcher.dispatchDiagnostic(e,t)}};function ie(d){let e=g.initialize();d.subscriptions.push(e),g.info("Sonar Agent extension activated.");let t=f.workspace.workspaceFolders?.[0]?.uri.fsPath,n=new U({secretStorage:d.secrets,workspaceConfig:{get:(r,s)=>f.workspace.getConfiguration("sonarAgent").get(r,s),update:(r,s,a)=>f.workspace.getConfiguration("sonarAgent").update(r,s,a)},workspaceRoot:t}),i=new D(d.extensionUri,n);d.subscriptions.push(f.window.registerWebviewViewProvider(D.viewType,i)),d.subscriptions.push(f.commands.registerCommand("sonarAgent.refresh",async()=>{await i.refresh()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.configure",async()=>{await i.promptConfigureConnection()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.selectProject",async()=>{await i.promptProjectSelection()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.profile.manage",async()=>{await i.promptManageProfiles()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.resetConnection",async()=>{await f.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await n.deleteToken(),await i.refresh(),f.window.showInformationMessage("SonarQube credentials have been removed."))})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.showLogs",()=>{g.show()})),n.migrateResetIfLegacy().then(r=>{r&&(g.info("Legacy single connection removed; directing user to create a profile."),f.window.showInformationMessage("Single connection removed \u2014 create a profile to reconnect.","New Profile").then(s=>{s==="New Profile"&&i.promptCreateProfile()}))});let o=new L({projectDetector:n});d.subscriptions.push(f.languages.registerCodeActionsProvider({scheme:"file"},o,{providedCodeActionKinds:L.providedCodeActionKinds})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.fixWithAgent",async(r,s)=>{!r||!s||await o.executeFixWithAgent(r,s)}))}function ne(){g.dispose()}0&&(module.exports={activate,deactivate});
