"use strict";var U=Object.create;var A=Object.defineProperty;var M=Object.getOwnPropertyDescriptor;var _=Object.getOwnPropertyNames;var $=Object.getPrototypeOf,N=Object.prototype.hasOwnProperty;var O=(p,e)=>{for(var t in e)A(p,t,{get:e[t],enumerable:!0})},D=(p,e,t,o)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of _(e))!N.call(p,n)&&n!==t&&A(p,n,{get:()=>e[n],enumerable:!(o=M(e,n))||o.enumerable});return p};var b=(p,e,t)=>(t=p!=null?U($(p)):{},D(e||!p||!p.__esModule?A(t,"default",{value:p,enumerable:!0}):t,p)),z=p=>D(A({},"__esModule",{value:!0}),p);var W={};O(W,{activate:()=>H,deactivate:()=>V});module.exports=z(W);var m=b(require("vscode"));var F=b(require("node:path")),L=b(require("node:fs/promises")),j="sonarAgent.token",E=class p{secrets;config;workspaceRoot;readFileFn;activeProjectKey;activeServerUrl;activeToken;constructor(e){this.secrets=e.secretStorage,this.config=e.workspaceConfig,this.workspaceRoot=e.workspaceRoot,this.readFileFn=e.readFileFn??(t=>L.readFile(t,"utf-8"))}static parseProperties(e){let t=e.split(/\r?\n/),o,n,i=!1;for(let r of t){let s=r.trim();if(!s||s.startsWith("#")||s.startsWith(";"))continue;let a=s.indexOf("=");if(a===-1)continue;let c=s.slice(0,a).trim(),d=s.slice(a+1).trim();c==="sonar.projectKey"?o=d:c==="sonar.host.url"?n=d:(c==="sonar.login"||c==="sonar.password"||c==="sonar.token")&&(i=!0)}return{projectKey:o,serverUrl:n,hasPlaintextCredentials:i}}async getToken(){if(this.activeToken!==void 0)return this.activeToken??void 0;let e=await this.secrets.get(j);return this.activeToken=e?e.trim():null,this.activeToken??void 0}async setToken(e){this.activeToken=e.trim(),await this.secrets.store(j,this.activeToken)}async deleteToken(){this.activeToken=null,await this.secrets.delete(j)}async setServerUrl(e){this.activeServerUrl=e.trim(),await this.config.update("serverUrl",this.activeServerUrl,!0)}async setProjectKey(e){this.activeProjectKey=e.trim(),await this.config.update("projectKey",this.activeProjectKey,!0)}async detectWorkspaceProperties(){if(!this.workspaceRoot)return null;let e=F.join(this.workspaceRoot,"sonar-project.properties");try{let t=await this.readFileFn(e);return p.parseProperties(t)}catch{return null}}async getConfig(){let e=this.activeServerUrl??this.config.get("serverUrl",""),t=this.activeProjectKey??this.config.get("projectKey",""),o=!1,n=!1,i=await this.detectWorkspaceProperties();i&&(i.projectKey&&(t=i.projectKey,o=!0),!e&&i.serverUrl&&(e=i.serverUrl),i.hasPlaintextCredentials&&(n=!0));let r=await this.getToken();return{serverUrl:e,projectKey:t,hasToken:!!(r&&r.trim().length>0),detectedFromProperties:o,hasPlaintextCredentialsWarning:n}}};var l=b(require("vscode"));var y=class{serverUrl;token;fetchFn;constructor(e){this.serverUrl=e.serverUrl.replace(/\/+$/,""),this.token=e.token?e.token.trim():void 0,this.fetchFn=e.fetchFn??globalThis.fetch}getAuthHeader(){return this.token?{Authorization:`Basic ${Buffer.from(`${this.token}:`).toString("base64")}`}:{}}parseRating(e){let t=typeof e=="number"?e:parseFloat(String(e||"1.0"));return t<=1?"A":t<=2?"B":t<=3?"C":t<=4?"D":"E"}extractFilePath(e){let t=e.indexOf(":");return t!==-1?e.slice(t+1):e}async authenticatedFetch(e,t=1e4){let o={Accept:"application/json",...this.getAuthHeader()},n=new AbortController,i=setTimeout(()=>n.abort(),t);try{let r=await this.fetchFn(e,{method:"GET",headers:o,signal:n.signal});if(r.status===401&&this.token){let s={Accept:"application/json",Authorization:`Bearer ${this.token}`},a=await this.fetchFn(e,{method:"GET",headers:s,signal:n.signal});if(a.ok)return a}return r}catch(r){throw r.name==="AbortError"||n.signal.aborted?new Error(`Connection timed out after ${t/1e3}s`,{cause:r}):r}finally{clearTimeout(i)}}async verifyConnection(){try{let e=`${this.serverUrl}/api/authentication/validate`,t=await this.authenticatedFetch(e);if(t.status===401||t.status===403)return{ok:!1,message:`Authentication failed (HTTP ${t.status}). Please verify your token.`};if(!t.ok)return{ok:!1,message:`Server returned HTTP ${t.status}: ${t.statusText}`};let o=await t.json();return o&&o.valid===!0?{ok:!0}:{ok:!1,message:"Invalid credentials: SonarQube reported token as invalid."}}catch(e){return{ok:!1,message:`Cannot reach SonarQube server at ${this.serverUrl}: ${e.message||String(e)}`}}}async fetchProjects(){let e=[`${this.serverUrl}/api/components/search?qualifiers=TRK&ps=100`,`${this.serverUrl}/api/projects/search?ps=100`,`${this.serverUrl}/api/components/search_projects?ps=100`,`${this.serverUrl}/api/projects/search?ps=100&qualifiers=TRK`,`${this.serverUrl}/api/components/search?qualifiers=TRK`];for(let t of e)try{let o=await this.authenticatedFetch(t,4e3);if(!o.ok)continue;let n=await o.json(),i=n.components||n.projects||n.results||(Array.isArray(n)?n:[]);if(Array.isArray(i)&&i.length>0)return i.map(r=>({key:r.key||r.id||r.projectKey,name:r.name||r.key}))}catch{}return[]}async getOverview(e){let t=["bugs","reliability_rating","vulnerabilities","security_rating","code_smells","sqale_rating","accepted_issues","wont_fix_issues","coverage","lines_to_cover","duplicated_lines_density","duplicated_lines","security_hotspots"].join(","),o=`${this.serverUrl}/api/measures/component?component=${encodeURIComponent(e)}&metricKeys=${t}`,n=await this.authenticatedFetch(o);if(!n.ok&&n.status===400)try{let c=((await n.clone().json())?.errors?.[0]?.msg||"").match(/The following metric keys are not found:\s*([^.]+)/i);if(c&&c[1]){let d=c[1].split(",").map(x=>x.trim()),u=t.split(",").filter(x=>!d.includes(x)).join(","),h=`${this.serverUrl}/api/measures/component?component=${encodeURIComponent(e)}&metricKeys=${u}`,w=await this.authenticatedFetch(h);w.ok&&(n=w)}}catch{}if(!n.ok)throw new Error(`Failed to fetch measures: HTTP ${n.status} ${n.statusText}`);let i=await n.json(),r={};for(let s of i.component?.measures||[])s.value!==void 0&&(r[s.metric]=s.value);return{security:{count:parseInt(r.vulnerabilities||"0",10),rating:this.parseRating(r.security_rating)},reliability:{count:parseInt(r.bugs||"0",10),rating:this.parseRating(r.reliability_rating)},maintainability:{count:parseInt(r.code_smells||"0",10),rating:this.parseRating(r.sqale_rating)},acceptedIssues:{count:parseInt(r.accepted_issues||r.wont_fix_issues||"0",10)},coverage:{percentage:parseFloat(r.coverage||"0"),linesToCover:parseInt(r.lines_to_cover||"0",10)},duplications:{percentage:parseFloat(r.duplicated_lines_density||"0"),duplicatedLines:parseInt(r.duplicated_lines||"0",10)},securityHotspots:{count:parseInt(r.security_hotspots||"0",10),rating:"A"}}}async getIssues(e,t){let o;if(t==="accepted")o=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=BUG,VULNERABILITY,CODE_SMELL&issueStatuses=ACCEPTED&ps=100`;else{let r="BUG,VULNERABILITY,CODE_SMELL";t==="reliability"?r="BUG":t==="security"?r="VULNERABILITY":t==="maintainability"&&(r="CODE_SMELL"),o=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=${r}&statuses=OPEN,CONFIRMED,REOPENED&ps=100`}let n=await this.authenticatedFetch(o);if(!n.ok&&t==="accepted"){let r=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=BUG,VULNERABILITY,CODE_SMELL&resolutions=WONTFIX&ps=100`,s=await this.authenticatedFetch(r);s.ok&&(n=s)}if(!n.ok)throw new Error(`Failed to fetch issues: HTTP ${n.status} ${n.statusText}`);return((await n.json()).issues||[]).map(r=>({id:r.key,ruleKey:r.rule||"",message:r.message||"",component:r.component||"",filePath:this.extractFilePath(r.component||""),line:r.line,type:r.type||"CODE_SMELL",severity:r.severity||"MAJOR",status:r.status||"OPEN",effort:r.effort,tags:r.tags||[],creationDate:r.creationDate||"",author:r.author||void 0}))}async getHotspots(e){let t=`${this.serverUrl}/api/hotspots/search?projectKey=${encodeURIComponent(e)}&status=TO_REVIEW&ps=100`,o=await this.authenticatedFetch(t);if(!o.ok)throw new Error(`Failed to fetch hotspots: HTTP ${o.status} ${o.statusText}`);return((await o.json()).hotspots||[]).map(i=>({id:i.key,ruleKey:i.ruleKey||"",message:i.message||"",component:i.component||"",filePath:this.extractFilePath(i.component||""),line:i.line,type:"HOTSPOT",severity:"MAJOR",status:i.status||"TO_REVIEW",tags:["security-hotspot"],creationDate:i.creationDate||"",author:i.author||void 0}))}async getEnrichedRule(e){try{let t=`${this.serverUrl}/api/rules/show?key=${encodeURIComponent(e)}`,o=await this.authenticatedFetch(t);if(!o.ok)return{key:e,name:e,cleanDesc:"Verify code adherence to Sonar rule guidelines."};let n=await o.json(),r=(n.rule?.mdDesc||n.rule?.htmlDesc||"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();return{key:n.rule?.key||e,name:n.rule?.name||e,cleanDesc:r||"Verify code adherence to Sonar rule guidelines."}}catch{return{key:e,name:e,cleanDesc:"Verify code adherence to Sonar rule guidelines."}}}async getCoverageFiles(e){let t=`${this.serverUrl}/api/measures/component_tree?component=${encodeURIComponent(e)}&metricKeys=uncovered_lines,coverage&qualifiers=FIL&strategy=leaves&s=metric&metricSort=uncovered_lines&asc=false&ps=100`,o=await this.authenticatedFetch(t);if(!o.ok)throw new Error(`Failed to fetch coverage files: HTTP ${o.status} ${o.statusText}`);let n=await o.json(),i=[];for(let r of n.components||[]){let s={};for(let d of r.measures||[])s[d.metric]=d.value;let a=parseInt(s.uncovered_lines||"0",10),c=parseFloat(s.coverage||"0");(a>0||c<80)&&i.push({id:r.key,ruleKey:"coverage:uncovered_lines",message:`${a} uncovered lines (${c.toFixed(0)}% coverage)`,component:r.key,filePath:r.path||this.extractFilePath(r.key),type:"COVERAGE",severity:c<50?"CRITICAL":"MAJOR",status:"UNCOVERED",effort:`${a} lines`,tags:["test-coverage","unit-test"],creationDate:new Date().toISOString()})}return i}async getDuplicationFiles(e){let t=`${this.serverUrl}/api/measures/component_tree?component=${encodeURIComponent(e)}&metricKeys=duplicated_lines_density,duplicated_blocks&qualifiers=FIL&strategy=leaves&s=metric&metricSort=duplicated_lines_density&asc=false&ps=100`,o=await this.authenticatedFetch(t);if(!o.ok)throw new Error(`Failed to fetch duplication files: HTTP ${o.status} ${o.statusText}`);let n=await o.json(),i=[];for(let r of n.components||[]){let s={};for(let d of r.measures||[])s[d.metric]=d.value;let a=parseFloat(s.duplicated_lines_density||"0"),c=parseInt(s.duplicated_blocks||"0",10);(a>0||c>0)&&i.push({id:r.key,ruleKey:"duplications:duplicated_code",message:`${a.toFixed(1)}% duplicated lines (${c} duplicated blocks)`,component:r.key,filePath:r.path||this.extractFilePath(r.key),type:"DUPLICATION",severity:a>20?"CRITICAL":"MAJOR",status:"DUPLICATED",effort:`${c} blocks`,tags:["code-duplication","refactoring"],creationDate:new Date().toISOString()})}return i}};var S=b(require("node:path")),T=b(require("node:fs/promises")),v=b(require("vscode")),C=class{workspaceRoot;fileExistsFn;findFilesFn;constructor(e){this.workspaceRoot=e?.workspaceRoot??v.workspace.workspaceFolders?.[0]?.uri.fsPath,this.fileExistsFn=e?.fileExistsFn??(async t=>{try{return await T.stat(t),!0}catch{return!1}}),this.findFilesFn=e?.findFilesFn??(async t=>(await v.workspace.findFiles(t,"**/node_modules/**",5)).map(n=>n.fsPath))}async resolveFilePath(e){if(!this.workspaceRoot)return null;let t=S.isAbsolute(e)?e:S.join(this.workspaceRoot,e);if(await this.fileExistsFn(t))return t;let o=S.basename(e);if(o){let n=await this.findFilesFn(`**/${o}`);if(n&&n.length>0)return n[0]}return null}async openFileAtLine(e,t){let o=await this.resolveFilePath(e);if(!o)return v.window.showWarningMessage(`Could not find file locally: ${e}`),!1;try{let n=v.Uri.file(o),i=await v.workspace.openTextDocument(n),r=await v.window.showTextDocument(i,{preview:!1});if(t!==void 0&&t>0){let s=t-1,a=new v.Position(s,0);r.selection=new v.Selection(a,a),r.revealRange(new v.Range(a,a),v.TextEditorRevealType.InCenter)}return!0}catch(n){return v.window.showErrorMessage(`Failed to open file: ${n.message||String(n)}`),!1}}};var B=b(require("node:path")),R=b(require("node:fs/promises")),g=b(require("vscode"));var k=class{ruleCache=new Map;fileNavigator;fetchRuleFn;readCodeSnippetFn;isExtensionInstalledFn;isAntigravityEnvFn;executeCommandFn;sendToAgentPanelFn;constructor(e){this.fileNavigator=e?.fileNavigator??new C,this.fetchRuleFn=e?.fetchRuleFn,this.readCodeSnippetFn=e?.readCodeSnippetFn,this.executeCommandFn=e?.executeCommandFn??((t,...o)=>g.commands.executeCommand(t,...o)),this.sendToAgentPanelFn=e?.sendToAgentPanelFn??(t=>{let o=g.antigravityExtensibility;return o&&typeof o.sendToAgentPanel=="function"?o.sendToAgentPanel(t):Promise.reject(new Error("antigravityExtensibility.sendToAgentPanel not available"))}),this.isExtensionInstalledFn=e?.isExtensionInstalledFn??(t=>{try{return!!g.extensions.getExtension(t)}catch{return!1}}),this.isAntigravityEnvFn=e?.isAntigravityEnvFn??(()=>{try{return!!((g.env.appName||"").toLowerCase().includes("antigravity")||process.env.GEMINI_CLI||process.env.ANTIGRAVITY_IDE||process.env.ANTIGRAVITY_AGENT)}catch{return!1}})}getAvailableAgents(){let e=[];return(this.isExtensionInstalledFn("github.copilot")||this.isExtensionInstalledFn("github.copilot-chat"))&&e.push({id:"copilot",name:"GitHub Copilot",description:"VS Code Copilot Chat",focusCommand:"workbench.action.chat.open"}),(this.isAntigravityEnvFn()||this.isExtensionInstalledFn("google.antigravity")||this.isExtensionInstalledFn("google.gemini"))&&e.push({id:"antigravity",name:"Antigravity Agent",description:"DeepMind Antigravity Agent",focusCommand:"antigravity.openChatView"}),this.isExtensionInstalledFn("anthropic.claude-code")&&e.push({id:"claude-code",name:"Claude Code",description:"Anthropic Claude Code for VS Code",focusCommand:"workbench.view.extension.claude-sidebar"}),this.isExtensionInstalledFn("saoudrizwan.claude-dev")&&e.push({id:"cline",name:"Cline",description:"Autonomous AI coding agent",focusCommand:"claude-dev.focus"}),this.isExtensionInstalledFn("rooveterinaryinc.roo-cline")&&e.push({id:"roo-code",name:"Roo Code",description:"Roo Code coding agent",focusCommand:"roo-cline.focus"}),this.isExtensionInstalledFn("continue.continue")&&e.push({id:"continue",name:"Continue",description:"Continue open-source AI assistant",focusCommand:"continue.focusContinueInputView"}),e.push({id:"clipboard",name:"Clipboard Only",description:"Copy prompt to clipboard"}),e}async getRule(e){if(this.ruleCache.has(e))return this.ruleCache.get(e);let t;return this.fetchRuleFn?t=await this.fetchRuleFn(e):t={key:e,name:e,cleanDesc:"Adhere to SonarQube quality standard for this rule."},this.ruleCache.set(e,t),t}detectLanguage(e){switch(B.extname(e).toLowerCase()){case".ts":case".tsx":return"typescript";case".js":case".jsx":return"javascript";case".vue":return"vue";case".html":return"html";case".css":return"css";case".py":return"python";case".java":return"java";case".go":return"go";case".rs":return"rust";default:return""}}async readCodeSnippet(e,t){if(this.readCodeSnippetFn)return this.readCodeSnippetFn(e,t);let o=await this.fileNavigator.resolveFilePath(e);if(!o)return null;try{let i=(await R.readFile(o,"utf-8")).split(/\r?\n/),r=i.length,s=t!==void 0&&t>0?t:1,a=Math.max(1,s-10),c=Math.min(r,s+10),d=[];for(let u=a;u<=c;u++){let h=i[u-1],w=u===s?" ---> [ISSUE HERE] ":"      ";d.push(`${u.toString().padStart(4," ")} |${w}${h}`)}return{snippet:d.join(`
...
`,i}let o=await this.getRule(e.ruleKey),n=`@workspace Please fix the following SonarQube issue:
...
`,n+=`- Sonar Rule: \`${o.key}\` - ${o.name}

`,n+=`### \u{1F4D6} SonarQube Rule Details
`,n+=`${o.cleanDesc}
`,o.recommendation&&(n+=`> Sonar Recommendation: ${o.recommendation}
`),n+=`
...
`,o=new Map;for(let n of e){let i=o.get(n.filePath)||[];i.push(n),o.set(n.filePath,i)}for(let[n,i]of o){t+=`## \u{1F4C1} File: \`${n}\` (${i.length} issues)

`;for(let r=0;r<i.length;r++){let s=i[r],a=await this.getRule(s.ruleKey),c=await this.readCodeSnippet(s.filePath,s.line);t+=`### Issue #${r+1}: Line ${s.line||"File level"} [${s.severity}] ${a.name}
`,t+=`- Message: "${s.message}"
`,t+=`- Rule: \`${a.key}\`
...
`,t}async dispatch(e,t,o,n){try{await g.env.clipboard.writeText(e)}catch{}if(o&&o.line&&await this.fileNavigator.openFileAtLine(o.filePath,o.line),t==="copilot")try{return await this.executeCommandFn("workbench.action.chat.open",{query:e}),g.window.showInformationMessage("Dispatched Fix Prompt to GitHub Copilot Chat!"),{ok:!0,message:"Dispatched to GitHub Copilot Chat."}}catch{return g.window.showInformationMessage("Prompt copied to clipboard! Paste it into GitHub Copilot Chat."),{ok:!0,message:"Copied to clipboard (Copilot chat command not found)."}}if(t==="antigravity"){try{let s=[],a=n&&n.length>0?n:o?[o]:[],c=new Set;for(let d of a)if(d.filePath){let u=await this.fileNavigator.resolveFilePath(d.filePath);if(u&&!c.has(u)){c.add(u);let h=d.line&&d.line>0?d.line-1:0;s.push({uri:g.Uri.file(u),startLine:h,endLine:h})}}return await this.sendToAgentPanelFn({message:e,files:s.length>0?s:void 0,autoSend:!1}),g.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}for(let s of["workbench.action.chat.open","antigravity.prioritized.chat.open","workbench.action.openChat"])try{return await this.executeCommandFn(s,{query:e}),g.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}try{return await this.executeCommandFn("antigravity.openChatView"),g.window.showInformationMessage("Antigravity Chat opened & prompt copied to clipboard! Press Cmd+V / Ctrl+V to paste."),{ok:!0,message:"Chat opened and prompt ready in clipboard."}}catch{return g.window.showInformationMessage("Fix Prompt copied to clipboard for Antigravity Agent! Paste it into your agent chat."),{ok:!0,message:"Prompt ready in clipboard for Antigravity Agent."}}}let i=this.getAvailableAgents().find(s=>s.id===t),r=i?.name||(t==="claude-code"?"Claude Code":t==="cline"?"Cline":t==="roo-code"?"Roo Code":t==="continue"?"Continue":"Clipboard");if(t==="claude-code"){let s=["workbench.view.extension.claude-sidebar","claude-code.focus","claude.focus"];for(let a of s)try{await this.executeCommandFn(a);break}catch{}}else if(i?.focusCommand)try{await this.executeCommandFn(i.focusCommand)}catch{}return t==="clipboard"?(g.window.showInformationMessage("Fix Prompt copied to clipboard!"),{ok:!0,message:"Prompt ready in clipboard."}):(g.window.showInformationMessage(`Fix Prompt copied to clipboard for ${r}! Paste it into your agent chat.`),{ok:!0,message:`Prompt ready in clipboard for ${r}.`})}};var I=class{constructor(e,t,o,n,i){this.extensionUri=e;this.projectDetector=t;this.fileNavigator=o??new C,this.agentDispatcher=n??new k({fileNavigator:this.fileNavigator}),this.diagnosticCollection=i??l.languages.createDiagnosticCollection("SonarQube")}static viewType="sonarAgent.overviewView";_view;fileNavigator;agentDispatcher;diagnosticCollection;resolveWebviewView(e,t,o){this._view=e,e.webview.options={enableScripts:!0,localResourceRoots:[this.extensionUri]},e.webview.html=this._getHtmlForWebview(e.webview),e.webview.onDidReceiveMessage(async n=>{try{switch(n.command){case"init":{await this._syncState();break}case"connect":{await this._handleConnect(n.serverUrl,n.token);break}case"disconnect":{await this._handleDisconnect();break}case"selectProject":{n.projectKey&&(await this.projectDetector.setProjectKey(n.projectKey),await this._syncState());break}case"openProjectPicker":{await this.promptProjectSelection();break}case"fetchDetails":{await this._handleFetchDetails(n.category);break}case"openFile":{await this.fileNavigator.openFileAtLine(n.filePath,n.line);break}case"sendToAgent":{await this._handleSendToAgent(n.item,n.targetAgentId);break}case"sendBatchToAgent":{await this._handleSendBatchToAgent(n.items,n.targetAgentId);break}case"setTargetAgent":{await l.workspace.getConfiguration("sonarAgent").update("defaultAgent",n.agentId,!0);break}case"refresh":{await this._syncState();break}}}catch(i){console.error("[SonarAgent] Webview message handling error:",i),this._view?.webview.postMessage({type:"error",message:i?.message||"Error processing request."})}}),this._syncState(),e.onDidChangeVisibility(()=>{e.visible&&this._syncState()})}async refresh(){this._view&&await this._syncState()}async promptProjectSelection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!e.serverUrl||!t){l.window.showWarningMessage("Please connect to SonarQube first.");return}let n=await new y({serverUrl:e.serverUrl,token:t}).fetchProjects(),i={label:"$(edit) Enter Project Key manually...",description:"Type the exact project key from SonarQube",detail:"Use this if your project is not listed or search is restricted"},r=[i,...n.map(a=>({label:a.name,description:a.key,detail:a.key===e.projectKey?"(Currently selected)":void 0}))],s=await l.window.showQuickPick(r,{placeHolder:"Select a SonarQube project or enter key manually",matchOnDescription:!0});if(s===i){let a=await l.window.showInputBox({prompt:"Enter the SonarQube Project Key",placeHolder:"e.g. org.company:my-project",value:e.projectKey||"",validateInput:c=>c.trim()?null:"Project Key cannot be empty"});if(a&&a.trim()){let c=a.trim();await this.projectDetector.setProjectKey(c),await this._syncState(),l.window.showInformationMessage(`Active SonarQube project set to: ${c}`)}return}s&&s.description&&(await this.projectDetector.setProjectKey(s.description),await this._syncState(),l.window.showInformationMessage(`Active SonarQube project set to: ${s.label}`))}async promptConfigureConnection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),o=!!(e.serverUrl&&t),n=[{label:"$(link) Update Server URL & Token",description:o?"Connected":"Not connected",detail:e.serverUrl?`Server: ${e.serverUrl}`:"Configure SonarQube host URL and authentication token",action:"updateCredentials"},{label:"$(folder-active) Select Sonar Project",description:e.projectKey?"Active":"Not selected",detail:e.projectKey?`Current project: ${e.projectKey}`:"Choose an active project on the server",action:"selectProject"},{label:"$(gear) Open Extension Settings",detail:"Configure default AI agent and advanced preferences",action:"openSettings"}];o&&n.push({label:"$(debug-disconnect) Disconnect & Reset Credentials",detail:"Remove stored token from OS Keychain and disconnect",action:"disconnect"});let i=await l.window.showQuickPick(n,{placeHolder:"Sonar Agent: Configure Connection & Settings",matchOnDescription:!0,matchOnDetail:!0});if(i)switch(i.action){case"updateCredentials":await this.promptUpdateCredentials(e.serverUrl);break;case"selectProject":await this.promptProjectSelection();break;case"openSettings":await l.commands.executeCommand("workbench.action.openSettings","@ext:chulit.sonar-agent");break;case"disconnect":await l.commands.executeCommand("sonarAgent.resetConnection");break}}async promptUpdateCredentials(e,t){let o=e||(await this.projectDetector.getConfig()).serverUrl||"http://localhost:9000",n=t||"";for(;;){let i=await l.window.showInputBox({title:"SonarQube Connection (1/2)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:o,ignoreFocusOut:!0,validateInput:c=>{let d=c.trim();if(!d)return"Server URL is required";if(!d.startsWith("http://")&&!d.startsWith("https://"))return"Server URL must start with http:// or https://";try{if(!new URL(d).hostname)return"Please enter a valid URL with hostname"}catch{return"Please enter a valid URL"}return null}});if(i===void 0)return;o=i.trim();let r=await l.window.showInputBox({title:"SonarQube Connection (2/2)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:n,password:!0,ignoreFocusOut:!0,validateInput:c=>c.trim()?null:"User Token is required"});if(r===void 0)return;n=r.trim();let s={ok:!1};if(await l.window.withProgress({location:l.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{s=await new y({serverUrl:o,token:n}).verifyConnection()}),!s.ok){if(await l.window.showErrorMessage(`SonarQube connection verification failed: ${s.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}await this.projectDetector.setServerUrl(o),await this.projectDetector.setToken(n),l.window.showInformationMessage("SonarQube connection successfully verified and saved!"),(await this.projectDetector.getConfig()).projectKey||await this.promptProjectSelection(),await this.refresh();return}}async _handleSendToAgent(e,t){let o=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),i=t||l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),r;o.serverUrl&&n&&(r=new y({serverUrl:o.serverUrl,token:n}));let s=new k({fileNavigator:this.fileNavigator,fetchRuleFn:r?c=>r.getEnrichedRule(c):void 0}),a=await s.assemblePrompt(e);await s.dispatch(a,i,e)}async _handleSendBatchToAgent(e,t){if(!e||e.length===0)return;let o=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),i=t||l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),r;o.serverUrl&&n&&(r=new y({serverUrl:o.serverUrl,token:n}));let s=new k({fileNavigator:this.fileNavigator,fetchRuleFn:r?c=>r.getEnrichedRule(c):void 0}),a=await s.assembleBatchPrompt(e);await s.dispatch(a,i,e[0],e)}async _handleFetchDetails(e){if(!this._view)return;let t=await this.projectDetector.getConfig(),o=await this.projectDetector.getToken();if(!(!t.serverUrl||!t.projectKey||!o)){this._view.webview.postMessage({type:"loadingDetails",loading:!0});try{let n=new y({serverUrl:t.serverUrl,token:o}),i=[];e==="hotspots"?i=await n.getHotspots(t.projectKey):e==="coverage"?i=await n.getCoverageFiles(t.projectKey):e==="duplications"?i=await n.getDuplicationFiles(t.projectKey):e==="reliability"||e==="security"||e==="maintainability"||e==="accepted"?i=await n.getIssues(t.projectKey,e):i=await n.getIssues(t.projectKey),this._view.webview.postMessage({type:"details",category:e,items:i}),await this.syncDiagnostics(i)}catch(n){this._view.webview.postMessage({type:"detailsError",message:n.message||"Failed to load issues."})}finally{this._view.webview.postMessage({type:"loadingDetails",loading:!1})}}}async syncDiagnostics(e){this.diagnosticCollection.clear();let t=new Map;for(let o of e){if(!o.filePath)continue;let n=await this.fileNavigator.resolveFilePath(o.filePath);if(!n)continue;let i=l.Uri.file(n),r=o.line&&o.line>0?o.line-1:0,s=new l.Range(r,0,r,100),a=l.DiagnosticSeverity.Information;o.severity==="BLOCKER"||o.severity==="CRITICAL"?a=l.DiagnosticSeverity.Error:o.severity==="MAJOR"&&(a=l.DiagnosticSeverity.Warning);let c=new l.Diagnostic(s,o.message,a);c.code=o.ruleKey,c.source="SonarQube";let d=t.get(i.toString())||{uri:i,diagnostics:[]};d.diagnostics.push(c),t.set(i.toString(),d)}for(let{uri:o,diagnostics:n}of t.values())this.diagnosticCollection.set(o,n)}async _syncState(){if(this._view)try{let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),o=l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),n=this.agentDispatcher.getAvailableAgents(),i=o;if(n.some(r=>r.id===i)||(i=n[0]?.id||"clipboard"),e.serverUrl&&e.hasToken&&t){this._view.webview.postMessage({type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:e.projectKey,detectedFromProperties:e.detectedFromProperties??!1,hasPlaintextWarning:e.hasPlaintextCredentialsWarning??!1,projects:[],defaultAgent:i,availableAgents:n}),this._view.webview.postMessage({type:"loading",loading:!0});let r=new y({serverUrl:e.serverUrl,token:t}),s=e.projectKey,[a,c]=await Promise.allSettled([r.fetchProjects(),s?r.getOverview(s):Promise.resolve(null)]),d=a.status==="fulfilled"?a.value:[];a.status==="rejected"&&console.error("[SonarAgent] fetchProjects error:",a.reason);let u=s;!u&&d.length>0&&(u=d[0].key,await this.projectDetector.setProjectKey(u));let h=null,w;if(u&&u===s)c.status==="fulfilled"?h=c.value:w=c.reason?.message||"Failed to fetch project measures.";else if(u)try{h=await r.getOverview(u)}catch(x){w=x.message||"Failed to fetch project measures."}this._view.webview.postMessage({type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:u,detectedFromProperties:e.detectedFromProperties??!1,hasPlaintextWarning:e.hasPlaintextCredentialsWarning??!1,projects:d,overview:h,overviewError:w,defaultAgent:i,availableAgents:n}),this._view.webview.postMessage({type:"loading",loading:!1})}else this._view.webview.postMessage({type:"state",state:"onboarding",serverUrl:e.serverUrl||"http://localhost:9000",defaultAgent:i,availableAgents:n})}catch(e){console.error("[SonarAgent] _syncState error:",e),this._view?.webview.postMessage({type:"error",message:e.message||"Failed to synchronize Sonar Agent state."}),this._view?.webview.postMessage({type:"loading",loading:!1})}}async _handleConnect(e,t){let o=(t||"").trim(),n=(e||"").trim();if(!n||!o){this._view?.webview.postMessage({type:"error",message:"Server URL and User Token are required."});return}!n.startsWith("http://")&&!n.startsWith("https://")&&(n="http://"+n),n=n.replace(/\/+$/,""),this._view?.webview.postMessage({type:"connecting"});try{let r=await new y({serverUrl:n,token:o}).verifyConnection();if(!r.ok){this._view?.webview.postMessage({type:"error",message:r.message||"Connection verification failed."});return}await this.projectDetector.setServerUrl(n),await this.projectDetector.setToken(o),l.window.showInformationMessage("SonarQube connection successfully verified!"),await this._syncState()}catch(i){console.error("[SonarAgent] _handleConnect error:",i),this._view?.webview.postMessage({type:"error",message:i?.message||"Unexpected connection error occurred."})}}async _handleDisconnect(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!!!(e.serverUrl&&t)){this._view?.webview.postMessage({type:"disconnected",message:"Connection credentials cleared."}),l.window.showInformationMessage("Sonar Agent connection inputs cleared.");return}await l.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await this.projectDetector.deleteToken(),this._view?.webview.postMessage({type:"disconnected",message:"Disconnected from SonarQube. Credentials removed."}),l.window.showInformationMessage("SonarQube credentials have been removed."),await this._syncState())}_getHtmlForWebview(e){let t=K();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${e.cspSource} 'unsafe-inline'; font-src ${e.cspSource}; img-src ${e.cspSource} https: data:; script-src 'nonce-${t}' ${e.cspSource};">
...
</html>`}};function K(){let p="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)p+=e.charAt(Math.floor(Math.random()*e.length));return p}var f=b(require("vscode"));var P=class{static providedCodeActionKinds=[f.CodeActionKind.QuickFix];projectDetector;fileNavigator;customDispatcher;constructor(e){this.projectDetector=e.projectDetector,this.fileNavigator=e.fileNavigator??new C,this.customDispatcher=e.dispatcher}provideCodeActions(e,t,o,n){let i=[];for(let r of o.diagnostics)if(r.source&&r.source.toLowerCase().includes("sonar")){let s=r.code?`\u26A1 Send to AI Agent (${r.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new f.CodeAction(s,f.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},a.diagnostics=[r],a.isPreferred=!0,i.push(a)}return i}async executeFixWithAgent(e,t){let o=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),i;o.serverUrl&&n&&(i=new y({serverUrl:o.serverUrl,token:n}));let r=this.customDispatcher??new k({fileNavigator:this.fileNavigator,fetchRuleFn:i?x=>i.getEnrichedRule(x):void 0}),s=await r.getAvailableAgents(),c=f.workspace.getConfiguration("sonarAgent").get("defaultAgent");(!c||!s.some(x=>x.id===c))&&(c=s[0]?.id||"antigravity");let d="MAJOR";e.severity===f.DiagnosticSeverity.Error?d="CRITICAL":e.severity===f.DiagnosticSeverity.Information?d="MINOR":e.severity===f.DiagnosticSeverity.Hint&&(d="INFO");let u=f.workspace.asRelativePath?f.workspace.asRelativePath(t.uri):t.fileName,h={id:String(e.code||"sonar-issue"),ruleKey:String(e.code||""),message:e.message,component:u,filePath:u,line:e.range.start.line+1,severity:d,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()},w=await r.assemblePrompt(h);return r.dispatch(w,c,h)}};function H(p){let e=m.workspace.workspaceFolders?.[0]?.uri.fsPath,t=new E({secretStorage:p.secrets,workspaceConfig:{get:(i,r)=>m.workspace.getConfiguration("sonarAgent").get(i,r),update:(i,r,s)=>m.workspace.getConfiguration("sonarAgent").update(i,r,s)},workspaceRoot:e}),o=new I(p.extensionUri,t);p.subscriptions.push(m.window.registerWebviewViewProvider(I.viewType,o)),p.subscriptions.push(m.commands.registerCommand("sonarAgent.refresh",async()=>{await o.refresh()})),p.subscriptions.push(m.commands.registerCommand("sonarAgent.configure",async()=>{await o.promptConfigureConnection()})),p.subscriptions.push(m.commands.registerCommand("sonarAgent.selectProject",async()=>{await o.promptProjectSelection()})),p.subscriptions.push(m.commands.registerCommand("sonarAgent.resetConnection",async()=>{await m.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await t.deleteToken(),await o.refresh(),m.window.showInformationMessage("SonarQube credentials have been removed."))}));let n=new P({projectDetector:t});p.subscriptions.push(m.languages.registerCodeActionsProvider({scheme:"file"},n,{providedCodeActionKinds:P.providedCodeActionKinds})),p.subscriptions.push(m.commands.registerCommand("sonarAgent.fixWithAgent",async(i,r)=>{!i||!r||await n.executeFixWithAgent(i,r)}))}function V(){}0&&(module.exports={activate,deactivate});
