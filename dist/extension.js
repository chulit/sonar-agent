...
</html>`}};function ne(){let l="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)l+=e.charAt(Math.floor(Math.random()*e.length));return l}var E=y(require("vscode"));var L=class{static providedCodeActionKinds=[E.CodeActionKind.QuickFix];dispatcher;constructor(e){this.dispatcher=e.dispatcher??new j({projectDetector:e.projectDetector,fileNavigator:e.fileNavigator})}provideCodeActions(e,t,n,i){let o=[];for(let r of n.diagnostics)if(r.source&&r.source.toLowerCase().includes("sonar")){let s=r.code?`\u26A1 Send to AI Agent (${r.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new E.CodeAction(s,E.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},a.diagnostics=[r],a.isPreferred=!0,o.push(a)}return o}async executeFixWithAgent(e,t){return this.dispatcher.dispatchDiagnostic(e,t)}};function oe(l){let e=u.initialize();l.subscriptions.push(e),u.info("Sonar Agent extension activated.");let t=h.workspace.workspaceFolders?.[0]?.uri.fsPath,n=new B({secretStorage:l.secrets,workspaceConfig:{get:(r,s)=>h.workspace.getConfiguration("sonarAgent").get(r,s),update:(r,s,a)=>h.workspace.getConfiguration("sonarAgent").update(r,s,a)},workspaceRoot:t}),i=new D(l.extensionUri,n);l.subscriptions.push(h.window.registerWebviewViewProvider(D.viewType,i)),l.subscriptions.push(h.commands.registerCommand("sonarAgent.refresh",async()=>{await i.refresh()})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.configure",async()=>{await i.promptConfigureConnection()})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.selectProject",async()=>{await i.promptProjectSelection()})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.profile.manage",async()=>{await i.promptManageProfiles()})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.resetConnection",async()=>{await h.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await n.deleteToken(),await i.refresh(),h.window.showInformationMessage("SonarQube credentials have been removed."))})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.showLogs",()=>{u.show()})),n.migrateResetIfLegacy().then(r=>{r&&(u.info("Legacy single connection removed; directing user to create a profile."),h.window.showInformationMessage("Single connection removed \u2014 create a profile to reconnect.","New Profile").then(s=>{s==="New Profile"&&i.promptCreateProfile()}))});let o=new L({projectDetector:n});l.subscriptions.push(h.languages.registerCodeActionsProvider({scheme:"file"},o,{providedCodeActionKinds:L.providedCodeActionKinds})),l.subscriptions.push(h.commands.registerCommand("sonarAgent.fixWithAgent",async(r,s)=>{!r||!s||await o.executeFixWithAgent(r,s)}))}function re(){u.dispose()}0&&(module.exports={activate,deactivate});
