...
`,t}async dispatchCopilot(e){try{return await this.executeCommandFn("workbench.action.chat.open",{query:e}),d.window.showInformationMessage("Dispatched Fix Prompt to GitHub Copilot Chat!"),{ok:!0,message:"Dispatched to GitHub Copilot Chat."}}catch{return d.window.showInformationMessage("Prompt copied to clipboard! Paste it into GitHub Copilot Chat."),{ok:!0,message:"Copied to clipboard (Copilot chat command not found)."}}}async collectAntigravityFiles(e){let t=[],n=new Set;for(let i of e){if(!i.filePath)continue;let o=await this.fileNavigator.resolveFilePath(i.filePath);if(o&&!n.has(o)){n.add(o);let r=i.line&&i.line>0?i.line-1:0;t.push({uri:d.Uri.file(o),startLine:r,endLine:r})}}return t}async tryOpenAntigravityChat(e){let t=["workbench.action.chat.open","antigravity.prioritized.chat.open","workbench.action.openChat"];for(let n of t)try{return await this.executeCommandFn(n,{query:e}),!0}catch{}return!1}async dispatchAntigravity(e,t,n){try{let i=[];n&&n.length>0?i=n:t&&(i=[t]);let o=await this.collectAntigravityFiles(i);return await this.sendToAgentPanelFn({message:e,files:o.length>0?o:void 0,autoSend:!1}),d.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}if(await this.tryOpenAntigravityChat(e))return d.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."};try{return await this.executeCommandFn("antigravity.openChatView"),d.window.showInformationMessage("Antigravity Chat opened & prompt copied to clipboard! Press Cmd+V / Ctrl+V to paste."),{ok:!0,message:"Chat opened and prompt ready in clipboard."}}catch{return d.window.showInformationMessage("Fix Prompt copied to clipboard for Antigravity Agent! Paste it into your agent chat."),{ok:!0,message:"Prompt ready in clipboard for Antigravity Agent."}}}async focusTargetAgent(e,t){if(e==="claude-code"){let n=["workbench.view.extension.claude-sidebar","claude-code.focus","claude.focus"];for(let i of n)try{await this.executeCommandFn(i);break}catch{}}else if(e==="codex"){let n=["chatgpt.focus","openai.chatgpt.focus","codex.focus"];for(let i of n)try{await this.executeCommandFn(i);break}catch{}}else if(t?.focusCommand)try{await this.executeCommandFn(t.focusCommand)}catch{}}async dispatch(e,t,n,i){try{await d.env.clipboard.writeText(e)}catch{}if(n?.line&&await this.fileNavigator.openFileAtLine(n.filePath,n.line),t==="copilot")return this.dispatchCopilot(e);if(t==="antigravity")return this.dispatchAntigravity(e,n,i);let o=this.getAvailableAgents().find(a=>a.id===t),r={"claude-code":"Claude Code",cline:"Cline","roo-code":"Roo Code",continue:"Continue",codex:"Codex Agent"},s=o?.name??r[t]??"Clipboard";return await this.focusTargetAgent(t,o),t==="clipboard"?(d.window.showInformationMessage("Fix Prompt copied to clipboard!"),{ok:!0,message:"Prompt ready in clipboard."}):(d.window.showInformationMessage(`Fix Prompt copied to clipboard for ${s}! Paste it into your agent chat.`),{ok:!0,message:`Prompt ready in clipboard for ${s}.`})}async resolveTargetAgent(e){let t=this.getAvailableAgents();if(e&&t.some(i=>i.id===e))return e;if(e==="clipboard")return"clipboard";let n=this.getDefaultAgentFn();return n&&t.some(i=>i.id===n)?n:t[0]?.id||"clipboard"}async dispatchIssue(e,t){let n=await this.resolveTargetAgent(t?.targetAgentId),i=await this.assemblePrompt(e);return this.dispatch(i,n,e)}async dispatchBatch(e,t){if(!e||e.length===0)return{ok:!1,message:"No items to dispatch."};let n=await this.resolveTargetAgent(t?.targetAgentId),i=await this.assembleBatchPrompt(e);return this.dispatch(i,n,e[0],e)}async dispatchDiagnostic(e,t,n){let i="MAJOR";e.severity===d.DiagnosticSeverity.Error?i="CRITICAL":e.severity===d.DiagnosticSeverity.Information?i="MINOR":e.severity===d.DiagnosticSeverity.Hint&&(i="INFO");let o=d.workspace.asRelativePath?d.workspace.asRelativePath(t.uri):t.fileName,r="";typeof e.code=="object"&&e.code!==null?r=String(e.code.value):e.code!==void 0&&e.code!==null&&(r=String(e.code));let s={id:r||"sonar-issue",ruleKey:r,message:e.message,component:o,filePath:o,line:e.range.start.line+1,severity:i,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()};return this.dispatchIssue(s,n)}};var w=h(require("vscode"));var W=h(require("vscode")),p=class{static channel;static initialize(e){return e?this.channel=e:this.channel??=W.window.createOutputChannel("Sonar Agent",{log:!0}),this.channel}static sanitize(e){if(!e)return"";let t=e.replace(/https?:\/\/[^\s"'`<>]+/gi,"[SERVER]");return t=t.replace(/(token|bearer|authorization|password)\s*[:=]\s*[^\s,;&]+/gi,"$1: [REDACTED]"),t}static info(e){this.channel||this.initialize(),this.channel?.info(this.sanitize(e))}static warn(e){this.channel||this.initialize(),this.channel?.warn(this.sanitize(e))}static error(e,t){this.channel||this.initialize();let n="";if(t!=null)if(t instanceof Error)n=` - ${t.message}`;else if(typeof t=="string"||typeof t=="number"||typeof t=="boolean")n=` - ${t}`;else try{n=` - ${JSON.stringify(t)}`}catch{n=" - [Object]"}this.channel?.error(this.sanitize(`${e}${n}`))}static debug(e){this.channel||this.initialize(),this.channel?.debug(this.sanitize(e))}static show(){this.channel?.show(!0)}static dispose(){this.channel?.dispose(),this.channel=void 0}};var U=class{constructor(e){this.options=e;this.projectDetector=e.projectDetector,this.onConfigChanged=e.onConfigChanged}projectDetector;onConfigChanged;get sonarClientFactory(){return this.options.sonarClientFactory??(e=>new y(e))}get showQuickPickFn(){return this.options.showQuickPickFn??w.window.showQuickPick}get showInputBoxFn(){return this.options.showInputBoxFn??w.window.showInputBox}get showInformationMessageFn(){return this.options.showInformationMessageFn??w.window.showInformationMessage}get showWarningMessageFn(){return this.options.showWarningMessageFn??w.window.showWarningMessage}get showErrorMessageFn(){return this.options.showErrorMessageFn??w.window.showErrorMessage}get withProgressFn(){return this.options.withProgressFn??w.window.withProgress}get executeCommandFn(){return this.options.executeCommandFn??w.commands.executeCommand}profilesCapable(){return typeof this.projectDetector.listProfiles=="function"}async notifyConfigChanged(){this.onConfigChanged&&await this.onConfigChanged()}async promptProjectSelection(){return this.options.promptProjectSelectionFn?this.options.promptProjectSelectionFn():this.promptProjectSelectionInternal()}async promptProjectSelectionInternal(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!e.serverUrl||!t){this.showWarningMessageFn("Please connect to SonarQube first.");return}let i=await this.sonarClientFactory({serverUrl:e.serverUrl,token:t}).fetchProjects(),o={label:"$(edit) Enter Project Key manually...",description:"Type the exact project key from SonarQube",detail:"Use this if your project is not listed or search is restricted"},r=[o,...i.map(a=>({label:a.name,description:a.key,detail:a.key===e.projectKey?"(Currently selected)":void 0}))],s=await this.showQuickPickFn(r,{placeHolder:"Select a SonarQube project or enter key manually",matchOnDescription:!0});if(s===o){let a=await this.showInputBoxFn({prompt:"Enter the SonarQube Project Key",placeHolder:"e.g. org.company:my-project",value:e.projectKey||"",validateInput:c=>c.trim()?null:"Project Key cannot be empty"});if(a?.trim()){let c=a.trim();await this.projectDetector.setProjectKey(c),await this.notifyConfigChanged(),this.showInformationMessageFn(`Active SonarQube project set to: ${c}`)}return}if(s?.description){let a=s.description;await this.projectDetector.setProjectKey(a),await this.notifyConfigChanged(),this.showInformationMessageFn(`Active SonarQube project set to: ${s.label}`)}}async promptConfigureConnection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),n=!!(e.serverUrl&&t),i=[{label:"$(link) Update Server URL & Token",description:n?"Connected":"Not connected",detail:e.serverUrl?`Server: ${e.serverUrl}`:"Configure SonarQube host URL and authentication token",action:"updateCredentials"},{label:"$(folder-active) Select Sonar Project",description:e.projectKey?"Active":"Not selected",detail:e.projectKey?`Current project: ${e.projectKey}`:"Choose an active project on the server",action:"selectProject"},{label:"$(settings-gear) Open Extension Settings",detail:"Configure default AI agent and advanced preferences",action:"openSettings"},{label:"$(output) Show Extension Logs",detail:"Open the Sonar Agent output log channel",action:"showLogs"}];n&&i.push({label:"$(debug-disconnect) Disconnect & Reset Credentials",detail:"Remove stored token from OS Keychain and disconnect",action:"disconnect"}),this.profilesCapable()&&(i.splice(2,0,{label:"$(arrow-swap) Switch Connection Profile",detail:"Activate a different server + project + token binding",action:"switchProfile"}),i.splice(3,0,{label:"$(add) New Connection Profile...",detail:"Create a named server + project binding with live verification",action:"newProfile"}),i.splice(4,0,{label:"$(edit) Rename Connection Profile...",detail:"Rename a stored connection profile",action:"renameProfile"}),i.splice(5,0,{label:"$(trash) Delete Connection Profile...",detail:"Remove a profile and delete its token from the OS Keychain",action:"deleteProfile"}),i.splice(6,0,{label:"$(check) Verify Active Connection",detail:"Live-check the active profile against the SonarQube server",action:"verifyConnection"}));let o=await this.showQuickPickFn(i,{placeHolder:"Sonar Agent: Configure Connection & Settings",matchOnDescription:!0,matchOnDetail:!0});if(!o)return;let r=o.action;switch(r){case"updateCredentials":await this.promptUpdateCredentials(e.serverUrl);break;case"selectProject":await this.promptProjectSelection();break;case"switchProfile":case"newProfile":case"renameProfile":case"deleteProfile":case"verifyConnection":await this.runProfileAction(r);break;case"openSettings":await this.executeCommandFn("workbench.action.openSettings","@ext:chulit.sonar-agent");break;case"showLogs":p.show();break;case"disconnect":await this.executeCommandFn("sonarAgent.resetConnection");break}}async promptUpdateCredentials(e,t){let n=e||(await this.projectDetector.getConfig()).serverUrl||"http://localhost:9000",i=t||"";for(;;){let o=await this.showInputBoxFn({title:"SonarQube Connection (1/2)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:n,ignoreFocusOut:!0,validateInput:c=>this.validateServerUrlInput(c)});if(o===void 0)return;n=o.trim();let r=await this.showInputBoxFn({title:"SonarQube Connection (2/2)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:i,password:!0,ignoreFocusOut:!0,validateInput:c=>c.trim()?null:"User Token is required"});if(r===void 0)return;i=r.trim();let s={ok:!1};if(await this.withProgressFn({location:w.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{s=await this.sonarClientFactory({serverUrl:n,token:i}).verifyConnection()}),!s.ok){if(await this.showErrorMessageFn(`SonarQube connection verification failed: ${s.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}await this.projectDetector.setServerUrl(n),await this.projectDetector.setToken(i),this.showInformationMessageFn("SonarQube connection successfully verified and saved!"),(await this.projectDetector.getConfig()).projectKey||await this.promptProjectSelection(),await this.notifyConfigChanged();return}}validateServerUrlInput(e){let t=e.trim();if(!t)return"Server URL is required";if(!t.startsWith("http://")&&!t.startsWith("https://"))return"Server URL must start with http:// or https://";try{if(!new URL(t).hostname)return"Please enter a valid URL with hostname"}catch{return"Please enter a valid URL"}return null}async promptManageProfiles(){let e=[{label:"$(arrow-swap) Switch Profile",action:"switchProfile"},{label:"$(add) New Profile...",action:"newProfile"},{label:"$(edit) Rename Profile...",action:"renameProfile"},{label:"$(trash) Delete Profile...",action:"deleteProfile"},{label:"$(check) Verify Active Connection",action:"verifyConnection"}],t=await this.showQuickPickFn(e,{placeHolder:"Sonar Agent: Manage Connection Profiles"});t&&await this.runProfileAction(t.action)}async runProfileAction(e){switch(e){case"switchProfile":await this.handleSwitchProfile();break;case"newProfile":await this.promptCreateProfile();break;case"renameProfile":await this.handleRenameProfile();break;case"deleteProfile":await this.handleDeleteProfile();break;case"verifyConnection":await this.handleVerifyActiveConnection();break}}async handleSwitchProfile(){let e=await this.pickProfile("Select the connection profile to activate");if(e){try{await this.projectDetector.activateProfile(e.id)}catch(t){let n=t?.message||`Unknown connection profile: ${e.id}`;this.showErrorMessageFn(n);return}this.showInformationMessageFn(`Active connection profile: ${e.name}`),await this.notifyConfigChanged()}}async handleRenameProfile(){let e=await this.pickProfile("Select the connection profile to rename");if(!e)return;let t=await this.showInputBoxFn({prompt:`New name for profile "${e.name}"`,value:e.name,ignoreFocusOut:!0,validateInput:n=>n.trim()?null:"Profile name is required"});t?.trim()&&(await this.projectDetector.renameProfile(e.id,t.trim()),await this.notifyConfigChanged())}async handleDeleteProfile(){let e=await this.pickProfile("Select the connection profile to delete");!e||await this.showWarningMessageFn(`Delete connection profile "${e.name}" and its stored token?`,{modal:!0},"Delete")!=="Delete"||(await this.projectDetector.deleteProfile(e.id),this.showInformationMessageFn(`Connection profile "${e.name}" deleted. Create a profile to reconnect.`),await this.notifyConfigChanged())}async handleVerifyActiveConnection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!e.serverUrl||!t){this.showWarningMessageFn("No active connection profile to verify.");return}let n=await this.verifyConnectionWithProgress(e.serverUrl,t);n.ok?this.showInformationMessageFn("SonarQube connection verified."):this.showErrorMessageFn(`SonarQube connection verification failed: ${n.message||"Unknown error"}`)}async verifyConnectionWithProgress(e,t){let n={ok:!1};return await this.withProgressFn({location:w.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{n=await this.sonarClientFactory({serverUrl:e,token:t}).verifyConnection()}),n}async pickProfile(e){let t=await this.projectDetector.listProfiles();if(t.length===0){this.showInformationMessageFn("No connection profiles yet. Create one first.");return}let n=await this.showQuickPickFn(t.map(i=>({label:i.name,description:i.id,detail:`${i.serverUrl} \xB7 ${i.projectKey}`})),{placeHolder:e,matchOnDescription:!0,matchOnDetail:!0});return n?t.find(i=>i.id===n.description):void 0}async promptProfileInputs(e){let t=await this.showInputBoxFn({title:"New Connection Profile (1/4)",prompt:"Name this profile (e.g. kantor-prod)",value:e.name,ignoreFocusOut:!0,validateInput:r=>r.trim()?null:"Profile name is required"});if(t===void 0)return;let n=await this.showInputBoxFn({title:"New Connection Profile (2/4)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:e.serverUrl,ignoreFocusOut:!0,validateInput:r=>this.validateServerUrlInput(r)});if(n===void 0)return;let i=await this.showInputBoxFn({title:"New Connection Profile (3/4)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:e.token,password:!0,ignoreFocusOut:!0,validateInput:r=>r.trim()?null:"User Token is required"});if(i===void 0)return;let o=await this.showInputBoxFn({title:"New Connection Profile (4/4)",prompt:"Enter the SonarQube Project Key (optional, pick later)",value:e.projectKey,ignoreFocusOut:!0});if(o!==void 0)return{name:t.trim(),serverUrl:n.trim(),token:i.trim(),projectKey:o.trim()}}async promptCreateProfile(){let e=await this.projectDetector.getCreationSuggestion(),t={name:"",serverUrl:e?.serverUrl??"http://localhost:9000",token:"",projectKey:e?.projectKey??""};for(;;){let n=await this.promptProfileInputs(t);if(!n)return;t=n;let i=await this.verifyConnectionWithProgress(t.serverUrl,t.token);if(!i.ok){if(await this.showErrorMessageFn(`SonarQube connection verification failed: ${i.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}await this.finalizeProfileCreation(t,e?.hasPlaintextCredentials);return}}async finalizeProfileCreation(e,t){let n=await this.projectDetector.createProfile(e);t&&this.showWarningMessageFn("sonar-project.properties contains plaintext credentials. They were not imported; remove them to avoid leaking secrets."),this.showInformationMessageFn(`Connection profile "${n.name}" created and activated.`),n.projectKey||await this.promptProjectSelection(),await this.notifyConfigChanged()}};var j=class{constructor(e,t,n,i,o,r){this.extensionUri=e;this.projectDetector=t;this.fileNavigator=n??new k,this.agentDispatcher=i??new I({fileNavigator:this.fileNavigator,projectDetector:this.projectDetector}),this.diagnosticCollection=o??g.languages.createDiagnosticCollection("SonarQube"),this.connectionWizard=r??new U({projectDetector:this.projectDetector,onConfigChanged:()=>this.refresh(),promptProjectSelectionFn:()=>this.promptProjectSelection()})}static viewType="sonarAgent.overviewView";_view;_webviewReady=!1;_lastStateMessage;fileNavigator;agentDispatcher;diagnosticCollection;connectionWizard;resolveWebviewView(e,t,n){this._view=e,this._webviewReady=!1;let i=this.projectDetector.isConfiguredSync?.()??!1;p.info(`[Host] resolveWebviewView called. visible=${e.visible}, isConfigured=${i}`),e.webview.options={enableScripts:!0,localResourceRoots:[this.extensionUri]},e.webview.html=this._getHtmlForWebview(e.webview,i),e.webview.onDidReceiveMessage(async o=>{try{let r=o.text?` text="${o.text}"`:"";switch(p.info(`[Host] onDidReceiveMessage: command=${o.command}${r}`),o.command){case"log":{p.info(`[Webview] ${o.text}`);break}case"ready":case"init":{this._webviewReady=!0,this._lastStateMessage&&this._view&&await this._view.webview.postMessage(this._lastStateMessage),await this._syncState();break}case"connect":{await this._handleConnect(o.serverUrl,o.token);break}case"disconnect":{await this._handleDisconnect();break}case"selectProject":{o.projectKey&&(await this.projectDetector.setProjectKey(o.projectKey),await this._syncState());break}case"switchProfile":{await this._handleSwitchProfile(o.profileId);break}case"createProfile":{await this.promptCreateProfile();break}case"openProjectPicker":{await this.promptProjectSelection();break}case"fetchDetails":{await this._handleFetchDetails(o.category);break}case"openFile":{await this.fileNavigator.openFileAtLine(o.filePath,o.line);break}case"sendToAgent":{await this._handleSendToAgent(o.item,o.targetAgentId);break}case"sendBatchToAgent":{await this._handleSendBatchToAgent(o.items,o.targetAgentId);break}case"setTargetAgent":{await g.workspace.getConfiguration("sonarAgent").update("defaultAgent",o.agentId,!0);break}case"refresh":{await this._syncState();break}}}catch(r){console.error("[SonarAgent] Webview message handling error:",r),this._view?.webview.postMessage({type:"error",message:r?.message||"Error processing request."})}}),this._syncState(),e.onDidChangeVisibility(()=>{e.visible&&this._syncState()})}async refresh(){this._view&&await this._syncState()}async promptProjectSelection(){return this.connectionWizard.promptProjectSelectionInternal()}async promptConfigureConnection(){return this.connectionWizard.promptConfigureConnection()}async promptUpdateCredentials(e,t){return this.connectionWizard.promptUpdateCredentials(e,t)}async promptManageProfiles(){return this.connectionWizard.promptManageProfiles()}async promptCreateProfile(){return this.connectionWizard.promptCreateProfile()}async _handleSendToAgent(e,t){await this.agentDispatcher.dispatchIssue(e,{targetAgentId:t})}async _handleSendBatchToAgent(e,t){await this.agentDispatcher.dispatchBatch(e,{targetAgentId:t})}async _handleFetchDetails(e){if(!this._view)return;let t=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken();if(!(!t.serverUrl||!t.projectKey||!n)){this._view.webview.postMessage({type:"loadingDetails",loading:!0});try{let i=new y({serverUrl:t.serverUrl,token:n}),o=[];e==="hotspots"?o=await i.getHotspots(t.projectKey):e==="coverage"?o=await i.getCoverageFiles(t.projectKey):e==="duplications"?o=await i.getDuplicationFiles(t.projectKey):e==="reliability"||e==="security"||e==="maintainability"||e==="accepted"?o=await i.getIssues(t.projectKey,e):o=await i.getIssues(t.projectKey),this._view.webview.postMessage({type:"details",category:e,items:o}),await this.syncDiagnostics(o)}catch(i){this._view.webview.postMessage({type:"detailsError",message:i.message||"Failed to load issues."})}finally{this._view.webview.postMessage({type:"loadingDetails",loading:!1})}}}async syncDiagnostics(e){this.diagnosticCollection.clear();let t=new Map;for(let n of e){if(!n.filePath)continue;let i=await this.fileNavigator.resolveFilePath(n.filePath);if(!i)continue;let o=g.Uri.file(i),r=n.line&&n.line>0?n.line-1:0,s=new g.Range(r,0,r,100),a=g.DiagnosticSeverity.Information;n.severity==="BLOCKER"||n.severity==="CRITICAL"?a=g.DiagnosticSeverity.Error:n.severity==="MAJOR"&&(a=g.DiagnosticSeverity.Warning);let c=new g.Diagnostic(s,n.message,a);c.code=n.ruleKey,c.source="SonarQube";let u=t.get(o.toString())||{uri:o,diagnostics:[]};u.diagnostics.push(c),t.set(o.toString(),u)}for(let{uri:n,diagnostics:i}of t.values())this.diagnosticCollection.set(n,i)}async _handleSwitchProfile(e){try{await this.projectDetector.activateProfile(e)}catch(t){this._view?.webview.postMessage({type:"error",message:t?.message||`Unknown connection profile: ${e}`});return}await this._syncState()}_getEffectiveDefaultAgent(){let e=g.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),t=this.agentDispatcher.getAvailableAgents();return t.some(n=>n.id===e)?e:t[0]?.id||"clipboard"}async _sendNoProfilesState(e,t,n,i){let o={type:"state",state:"no-profiles",serverUrl:"http://localhost:9000",profiles:e,activeProfileId:t,defaultAgent:n,availableAgents:i};this._lastStateMessage=o,await this._view?.webview.postMessage(o)}async _sendOnboardingState(e,t,n,i,o){let r={type:"state",state:"onboarding",serverUrl:e||"http://localhost:9000",profiles:t,activeProfileId:n,defaultAgent:i,availableAgents:o};this._lastStateMessage=r;let s=await this._view?.webview.postMessage(r);p.info(`[Host] postMessage(onboarding) delivered=${s}`)}async _fetchOverviewForProject(e,t){if(!t)return{overview:null};try{let n=await e.getOverview(t);return n&&p.info(`Measures updated for [${t}]: ${n.security.count} vulnerabilities, ${n.reliability.count} bugs, ${n.maintainability.count} smells, ${n.coverage.percentage.toFixed(1)}% coverage.`),{overview:n}}catch(n){let i=n.message||"Failed to fetch project measures.";return p.error(`Failed to fetch project measures for [${t}]`,i),{overview:null,overviewError:i}}}async _resolveOverview(e,t,n,i){if(!t)return{overview:null};if(t===n&&i){if(i.status==="fulfilled"){let r=i.value;return r&&p.info(`Measures updated for [${t}]: ${r.security.count} vulnerabilities, ${r.reliability.count} bugs, ${r.maintainability.count} smells, ${r.coverage.percentage.toFixed(1)}% coverage.`),{overview:r}}let o=i.reason?.message||"Failed to fetch project measures.";return p.error(`Failed to fetch project measures for [${t}]`,o),{overview:null,overviewError:o}}return this._fetchOverviewForProject(e,t)}async _syncConnectedState(e){let{config:t,token:n,profiles:i,activeProfileId:o,effectiveDefaultAgent:r,availableAgents:s}=e,a={type:"state",state:"connected",serverUrl:t.serverUrl,projectKey:t.projectKey,projects:[],profiles:i,activeProfileId:o,defaultAgent:r,availableAgents:s};this._lastStateMessage=a;let c=await this._view?.webview.postMessage(a);p.info(`[Host] Immediate postMessage(connected) delivered=${c}`),await this._view?.webview.postMessage({type:"loading",loading:!0});let u=new y({serverUrl:t.serverUrl,token:n}),m=t.projectKey,[b,P]=await Promise.allSettled([u.fetchProjects(),m?u.getOverview(m):Promise.resolve(null)]),x=b.status==="fulfilled"?b.value:[];b.status==="rejected"&&console.error("[SonarAgent] fetchProjects error:",b.reason);let A=m;!A&&x.length>0&&(A=x[0].key,await this.projectDetector.setProjectKey(A));let{overview:H,overviewError:Q}=await this._resolveOverview(u,A,m,P),R={type:"state",state:"connected",serverUrl:t.serverUrl,projectKey:A,projects:x,overview:H,overviewError:Q,profiles:i,activeProfileId:o,defaultAgent:r,availableAgents:s};this._lastStateMessage=R;let G=await this._view?.webview.postMessage(R);p.info(`[Host] Full postMessage(connected) delivered=${G}`),await this._view?.webview.postMessage({type:"loading",loading:!1}),!this._webviewReady&&this._view&&setTimeout(async()=>{!this._webviewReady&&this._view&&this._lastStateMessage&&await this._view.webview.postMessage(this._lastStateMessage)},350)}async _syncState(){if(this._view)try{let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),n=this._getEffectiveDefaultAgent(),i=this.agentDispatcher.getAvailableAgents(),o=await this.projectDetector.listProfiles?.()??[],r=await this.projectDetector.getActiveProfile?.()??null;if(p.info(`[Host] _syncState: serverUrl=${e.serverUrl}, hasToken=${e.hasToken}, tokenPresent=${!!t}`),o.length===0&&!e.serverUrl&&!t){await this._sendNoProfilesState(o,r?.id,n,i);return}e.serverUrl&&e.hasToken&&t?await this._syncConnectedState({config:e,token:t,profiles:o,activeProfileId:r?.id,effectiveDefaultAgent:n,availableAgents:i}):await this._sendOnboardingState(e.serverUrl,o,r?.id,n,i)}catch(e){console.error("[SonarAgent] _syncState error:",e),p.error("Failed to synchronize Sonar Agent state",e),this._view?.webview.postMessage({type:"error",message:e.message||"Failed to synchronize Sonar Agent state."}),this._view?.webview.postMessage({type:"loading",loading:!1})}}async _handleConnect(e,t){let n=(t||"").trim(),i=(e||"").trim();if(!i||!n){this._view?.webview.postMessage({type:"error",message:"Server URL and User Token are required."});return}for(!i.startsWith("http://")&&!i.startsWith("https://")&&(i="https://"+i);i.endsWith("/");)i=i.slice(0,-1);this._view?.webview.postMessage({type:"connecting"}),p.info("Verifying SonarQube credentials...");try{let r=await new y({serverUrl:i,token:n}).verifyConnection();if(!r.ok){p.warn(`Connection verification failed: ${r.message||"Unknown error"}`),this._view?.webview.postMessage({type:"error",message:r.message||"Connection verification failed."});return}await this.projectDetector.setServerUrl(i),await this.projectDetector.setToken(n),p.info("SonarQube connection verified and saved."),g.window.showInformationMessage("SonarQube connection successfully verified!"),await this._syncState()}catch(o){console.error("[SonarAgent] _handleConnect error:",o),p.error("Unexpected connection error occurred",o),this._view?.webview.postMessage({type:"error",message:o?.message||"Unexpected connection error occurred."})}}async _handleDisconnect(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!!!(e.serverUrl&&t)){this._view?.webview.postMessage({type:"disconnected",message:"Connection credentials cleared."}),g.window.showInformationMessage("Sonar Agent connection inputs cleared.");return}await g.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await this.projectDetector.deleteToken(),p.info("SonarQube credentials removed and disconnected."),this._view?.webview.postMessage({type:"disconnected",message:"Disconnected from SonarQube. Credentials removed."}),g.window.showInformationMessage("SonarQube credentials have been removed."),await this._syncState())}_getHtmlForWebview(e,t=!1){let n=oe();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};function oe(){return V.randomBytes(16).toString("hex")}var F=h(require("vscode"));var D=class{static providedCodeActionKinds=[F.CodeActionKind.QuickFix];dispatcher;constructor(e){this.dispatcher=e.dispatcher??new I({projectDetector:e.projectDetector,fileNavigator:e.fileNavigator})}provideCodeActions(e,t,n,i){let o=[];for(let r of n.diagnostics)if(r.source?.toLowerCase().includes("sonar")){let s=typeof r.code=="object"&&r.code!==null?r.code.value:r.code,a=s?`\u26A1 Send to AI Agent (${s})`:"\u26A1 Send to AI Agent (SonarQube)",c=new F.CodeAction(a,F.CodeActionKind.QuickFix);c.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},c.diagnostics=[r],c.isPreferred=!0,o.push(c)}return o}async executeFixWithAgent(e,t){return this.dispatcher.dispatchDiagnostic(e,t)}};function re(l){let e=p.initialize();l.subscriptions.push(e),p.info("Sonar Agent extension activated.");let t=v.workspace.workspaceFolders?.[0]?.uri.fsPath,n=new L({secretStorage:l.secrets,workspaceConfig:{get:(r,s)=>v.workspace.getConfiguration("sonarAgent").get(r,s),update:(r,s,a)=>v.workspace.getConfiguration("sonarAgent").update(r,s,a)},workspaceRoot:t}),i=new j(l.extensionUri,n),o=new D({projectDetector:n});l.subscriptions.push(v.window.registerWebviewViewProvider(j.viewType,i),v.commands.registerCommand("sonarAgent.refresh",async()=>{await i.refresh()}),v.commands.registerCommand("sonarAgent.configure",async()=>{await i.promptConfigureConnection()}),v.commands.registerCommand("sonarAgent.selectProject",async()=>{await i.promptProjectSelection()}),v.commands.registerCommand("sonarAgent.profile.manage",async()=>{await i.promptManageProfiles()}),v.commands.registerCommand("sonarAgent.resetConnection",async()=>{await v.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await n.deleteToken(),await i.refresh(),v.window.showInformationMessage("SonarQube credentials have been removed."))}),v.commands.registerCommand("sonarAgent.showLogs",()=>{p.show()}),v.languages.registerCodeActionsProvider({scheme:"file"},o,{providedCodeActionKinds:D.providedCodeActionKinds}),v.commands.registerCommand("sonarAgent.fixWithAgent",async(r,s)=>{!r||!s||await o.executeFixWithAgent(r,s)})),n.migrateResetIfLegacy().then(r=>{r&&(p.info("Legacy single connection removed; directing user to create a profile."),v.window.showInformationMessage("Single connection removed \u2014 create a profile to reconnect.","New Profile").then(s=>{s==="New Profile"&&i.promptCreateProfile()}))})}function se(){p.dispose()}0&&(module.exports={activate,deactivate});
