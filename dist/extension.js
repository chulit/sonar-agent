...
</html>`}};function te(){let d="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)d+=e.charAt(Math.floor(Math.random()*e.length));return d}var b=P(require("vscode"));var L=class{static providedCodeActionKinds=[b.CodeActionKind.QuickFix];projectDetector;fileNavigator;customDispatcher;constructor(e){this.projectDetector=e.projectDetector,this.fileNavigator=e.fileNavigator??new S,this.customDispatcher=e.dispatcher}provideCodeActions(e,t,n,i){let o=[];for(let r of n.diagnostics)if(r.source&&r.source.toLowerCase().includes("sonar")){let s=r.code?`\u26A1 Send to AI Agent (${r.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new b.CodeAction(s,b.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},a.diagnostics=[r],a.isPreferred=!0,o.push(a)}return o}async executeFixWithAgent(e,t){let n=await this.projectDetector.getConfig(),i=await this.projectDetector.getToken(),o;n.serverUrl&&i&&(o=new w({serverUrl:n.serverUrl,token:i}));let r=this.customDispatcher??new I({fileNavigator:this.fileNavigator,fetchRuleFn:o?k=>o.getEnrichedRule(k):void 0}),s=await r.getAvailableAgents(),l=b.workspace.getConfiguration("sonarAgent").get("defaultAgent");(!l||!s.some(k=>k.id===l))&&(l=s[0]?.id||"antigravity");let p="MAJOR";e.severity===b.DiagnosticSeverity.Error?p="CRITICAL":e.severity===b.DiagnosticSeverity.Information?p="MINOR":e.severity===b.DiagnosticSeverity.Hint&&(p="INFO");let g=b.workspace.asRelativePath?b.workspace.asRelativePath(t.uri):t.fileName,v={id:String(e.code||"sonar-issue"),ruleKey:String(e.code||""),message:e.message,component:g,filePath:g,line:e.range.start.line+1,severity:p,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()},C=await r.assemblePrompt(v);return r.dispatch(C,l,v)}};function ie(d){let e=u.initialize();d.subscriptions.push(e),u.info("Sonar Agent extension activated.");let t=h.workspace.workspaceFolders?.[0]?.uri.fsPath,n=new U({secretStorage:d.secrets,workspaceConfig:{get:(r,s)=>h.workspace.getConfiguration("sonarAgent").get(r,s),update:(r,s,a)=>h.workspace.getConfiguration("sonarAgent").update(r,s,a)},workspaceRoot:t}),i=new D(d.extensionUri,n);d.subscriptions.push(h.window.registerWebviewViewProvider(D.viewType,i)),d.subscriptions.push(h.commands.registerCommand("sonarAgent.refresh",async()=>{await i.refresh()})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.configure",async()=>{await i.promptConfigureConnection()})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.selectProject",async()=>{await i.promptProjectSelection()})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.profile.manage",async()=>{await i.promptManageProfiles()})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.resetConnection",async()=>{await h.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await n.deleteToken(),await i.refresh(),h.window.showInformationMessage("SonarQube credentials have been removed."))})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.showLogs",()=>{u.show()})),n.migrateResetIfLegacy().then(r=>{r&&(u.info("Legacy single connection removed; directing user to create a profile."),h.window.showInformationMessage("Single connection removed \u2014 create a profile to reconnect.","New Profile").then(s=>{s==="New Profile"&&i.promptCreateProfile()}))});let o=new L({projectDetector:n});d.subscriptions.push(h.languages.registerCodeActionsProvider({scheme:"file"},o,{providedCodeActionKinds:L.providedCodeActionKinds})),d.subscriptions.push(h.commands.registerCommand("sonarAgent.fixWithAgent",async(r,s)=>{!r||!s||await o.executeFixWithAgent(r,s)}))}function ne(){u.dispose()}0&&(module.exports={activate,deactivate});
