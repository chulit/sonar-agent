"use strict";var $=Object.create;var E=Object.defineProperty;var _=Object.getOwnPropertyDescriptor;var N=Object.getOwnPropertyNames;var O=Object.getPrototypeOf,z=Object.prototype.hasOwnProperty;var K=(d,e)=>{for(var t in e)E(d,t,{get:e[t],enumerable:!0})},L=(d,e,t,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of N(e))!z.call(d,n)&&n!==t&&E(d,n,{get:()=>e[n],enumerable:!(i=_(e,n))||i.enumerable});return d};var w=(d,e,t)=>(t=d!=null?$(O(d)):{},L(e||!d||!d.__esModule?E(t,"default",{value:d,enumerable:!0}):t,d)),H=d=>L(E({},"__esModule",{value:!0}),d);var G={};K(G,{activate:()=>W,deactivate:()=>Q});module.exports=H(G);var f=w(require("vscode"));var F=w(require("node:path")),T=w(require("node:fs/promises")),D="sonarAgent.token",j=class d{secrets;config;workspaceRoot;readFileFn;activeProjectKey;activeServerUrl;activeToken;constructor(e){this.secrets=e.secretStorage,this.config=e.workspaceConfig,this.workspaceRoot=e.workspaceRoot,this.readFileFn=e.readFileFn??(t=>T.readFile(t,"utf-8"))}static parseProperties(e){let t=e.split(/\r?\n/),i,n,o=!1;for(let r of t){let s=r.trim();if(!s||s.startsWith("#")||s.startsWith(";"))continue;let a=s.indexOf("=");if(a===-1)continue;let c=s.slice(0,a).trim(),p=s.slice(a+1).trim();c==="sonar.projectKey"?i=p:c==="sonar.host.url"?n=p:(c==="sonar.login"||c==="sonar.password"||c==="sonar.token")&&(o=!0)}return{projectKey:i,serverUrl:n,hasPlaintextCredentials:o}}async getToken(){if(this.activeToken!==void 0)return this.activeToken??void 0;let e=await this.secrets.get(D);return this.activeToken=e?e.trim():null,this.activeToken??void 0}async setToken(e){this.activeToken=e.trim(),await this.secrets.store(D,this.activeToken)}async deleteToken(){this.activeToken=null,await this.secrets.delete(D)}async setServerUrl(e){this.activeServerUrl=e.trim(),await this.config.update("serverUrl",this.activeServerUrl,!0)}async setProjectKey(e){this.activeProjectKey=e.trim(),await this.config.update("projectKey",this.activeProjectKey,!0)}async detectWorkspaceProperties(){if(!this.workspaceRoot)return null;let e=F.join(this.workspaceRoot,"sonar-project.properties");try{let t=await this.readFileFn(e);return d.parseProperties(t)}catch{return null}}async getConfig(){let e=this.activeServerUrl??this.config.get("serverUrl",""),t=this.activeProjectKey??this.config.get("projectKey",""),i=!1,n=!1,o=await this.detectWorkspaceProperties();o&&(o.projectKey&&(t=o.projectKey,i=!0),!e&&o.serverUrl&&(e=o.serverUrl),o.hasPlaintextCredentials&&(n=!0));let r=await this.getToken();return{serverUrl:e,projectKey:t,hasToken:!!(r&&r.trim().length>0),detectedFromProperties:i,hasPlaintextCredentialsWarning:n}}};var l=w(require("vscode"));var x=class{serverUrl;token;fetchFn;constructor(e){this.serverUrl=e.serverUrl.replace(/\/+$/,""),this.token=e.token?e.token.trim():void 0,this.fetchFn=e.fetchFn??globalThis.fetch}getAuthHeader(){return this.token?{Authorization:`Basic ${Buffer.from(`${this.token}:`).toString("base64")}`}:{}}parseRating(e){let t=typeof e=="number"?e:parseFloat(String(e||"1.0"));return t<=1?"A":t<=2?"B":t<=3?"C":t<=4?"D":"E"}extractFilePath(e){let t=e.indexOf(":");return t!==-1?e.slice(t+1):e}async authenticatedFetch(e,t=1e4){let i={Accept:"application/json",...this.getAuthHeader()},n=new AbortController,o=setTimeout(()=>n.abort(),t);try{let r=await this.fetchFn(e,{method:"GET",headers:i,signal:n.signal});if(r.status===401&&this.token){let s={Accept:"application/json",Authorization:`Bearer ${this.token}`},a=await this.fetchFn(e,{method:"GET",headers:s,signal:n.signal});if(a.ok)return a}return r}catch(r){throw r.name==="AbortError"||n.signal.aborted?new Error(`Connection timed out after ${t/1e3}s`,{cause:r}):r}finally{clearTimeout(o)}}async verifyConnection(){try{let e=`${this.serverUrl}/api/authentication/validate`,t=await this.authenticatedFetch(e);if(t.status===401||t.status===403)return{ok:!1,message:`Authentication failed (HTTP ${t.status}). Please verify your token.`};if(!t.ok)return{ok:!1,message:`Server returned HTTP ${t.status}: ${t.statusText}`};let i=await t.json();return i&&i.valid===!0?{ok:!0}:{ok:!1,message:"Invalid credentials: SonarQube reported token as invalid."}}catch(e){return{ok:!1,message:`Cannot reach SonarQube server at ${this.serverUrl}: ${e.message||String(e)}`}}}async fetchProjects(){let e=[`${this.serverUrl}/api/components/search?qualifiers=TRK&ps=100`,`${this.serverUrl}/api/projects/search?ps=100`,`${this.serverUrl}/api/components/search_projects?ps=100`,`${this.serverUrl}/api/projects/search?ps=100&qualifiers=TRK`,`${this.serverUrl}/api/components/search?qualifiers=TRK`];for(let t of e)try{let i=await this.authenticatedFetch(t,4e3);if(!i.ok)continue;let n=await i.json(),o=n.components||n.projects||n.results||(Array.isArray(n)?n:[]);if(Array.isArray(o)&&o.length>0)return o.map(r=>({key:r.key||r.id||r.projectKey,name:r.name||r.key}))}catch{}return[]}async getOverview(e){let t=["bugs","reliability_rating","vulnerabilities","security_rating","code_smells","sqale_rating","accepted_issues","wont_fix_issues","coverage","lines_to_cover","duplicated_lines_density","duplicated_lines","security_hotspots"].join(","),i=`${this.serverUrl}/api/measures/component?component=${encodeURIComponent(e)}&metricKeys=${t}`,n=await this.authenticatedFetch(i);if(!n.ok&&n.status===400)try{let c=((await n.clone().json())?.errors?.[0]?.msg||"").match(/The following metric keys are not found:\s*([^.]+)/i);if(c&&c[1]){let p=c[1].split(",").map(C=>C.trim()),u=t.split(",").filter(C=>!p.includes(C)).join(","),g=`${this.serverUrl}/api/measures/component?component=${encodeURIComponent(e)}&metricKeys=${u}`,b=await this.authenticatedFetch(g);b.ok&&(n=b)}}catch{}if(!n.ok)throw new Error(`Failed to fetch measures: HTTP ${n.status} ${n.statusText}`);let o=await n.json(),r={};for(let s of o.component?.measures||[])s.value!==void 0&&(r[s.metric]=s.value);return{security:{count:parseInt(r.vulnerabilities||"0",10),rating:this.parseRating(r.security_rating)},reliability:{count:parseInt(r.bugs||"0",10),rating:this.parseRating(r.reliability_rating)},maintainability:{count:parseInt(r.code_smells||"0",10),rating:this.parseRating(r.sqale_rating)},acceptedIssues:{count:parseInt(r.accepted_issues||r.wont_fix_issues||"0",10)},coverage:{percentage:parseFloat(r.coverage||"0"),linesToCover:parseInt(r.lines_to_cover||"0",10)},duplications:{percentage:parseFloat(r.duplicated_lines_density||"0"),duplicatedLines:parseInt(r.duplicated_lines||"0",10)},securityHotspots:{count:parseInt(r.security_hotspots||"0",10),rating:"A"}}}async getIssues(e,t){let i;if(t==="accepted")i=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=BUG,VULNERABILITY,CODE_SMELL&issueStatuses=ACCEPTED&ps=100`;else{let r="BUG,VULNERABILITY,CODE_SMELL";t==="reliability"?r="BUG":t==="security"?r="VULNERABILITY":t==="maintainability"&&(r="CODE_SMELL"),i=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=${r}&statuses=OPEN,CONFIRMED,REOPENED&ps=100`}let n=await this.authenticatedFetch(i);if(!n.ok&&t==="accepted"){let r=`${this.serverUrl}/api/issues/search?componentKeys=${encodeURIComponent(e)}&types=BUG,VULNERABILITY,CODE_SMELL&resolutions=WONTFIX&ps=100`,s=await this.authenticatedFetch(r);s.ok&&(n=s)}if(!n.ok)throw new Error(`Failed to fetch issues: HTTP ${n.status} ${n.statusText}`);return((await n.json()).issues||[]).map(r=>({id:r.key,ruleKey:r.rule||"",message:r.message||"",component:r.component||"",filePath:this.extractFilePath(r.component||""),line:r.line,type:r.type||"CODE_SMELL",severity:r.severity||"MAJOR",status:r.status||"OPEN",effort:r.effort,tags:r.tags||[],creationDate:r.creationDate||"",author:r.author||void 0}))}async getHotspots(e){let t=`${this.serverUrl}/api/hotspots/search?projectKey=${encodeURIComponent(e)}&status=TO_REVIEW&ps=100`,i=await this.authenticatedFetch(t);if(!i.ok)throw new Error(`Failed to fetch hotspots: HTTP ${i.status} ${i.statusText}`);return((await i.json()).hotspots||[]).map(o=>({id:o.key,ruleKey:o.ruleKey||"",message:o.message||"",component:o.component||"",filePath:this.extractFilePath(o.component||""),line:o.line,type:"HOTSPOT",severity:"MAJOR",status:o.status||"TO_REVIEW",tags:["security-hotspot"],creationDate:o.creationDate||"",author:o.author||void 0}))}async getEnrichedRule(e){try{let t=`${this.serverUrl}/api/rules/show?key=${encodeURIComponent(e)}`,i=await this.authenticatedFetch(t);if(!i.ok)return{key:e,name:e,cleanDesc:"Verify code adherence to Sonar rule guidelines."};let n=await i.json(),r=(n.rule?.mdDesc||n.rule?.htmlDesc||"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();return{key:n.rule?.key||e,name:n.rule?.name||e,cleanDesc:r||"Verify code adherence to Sonar rule guidelines."}}catch{return{key:e,name:e,cleanDesc:"Verify code adherence to Sonar rule guidelines."}}}async getCoverageFiles(e){let t=`${this.serverUrl}/api/measures/component_tree?component=${encodeURIComponent(e)}&metricKeys=uncovered_lines,coverage&qualifiers=FIL&strategy=leaves&s=metric&metricSort=uncovered_lines&asc=false&ps=100`,i=await this.authenticatedFetch(t);if(!i.ok)throw new Error(`Failed to fetch coverage files: HTTP ${i.status} ${i.statusText}`);let n=await i.json(),o=[];for(let r of n.components||[]){let s={};for(let p of r.measures||[])s[p.metric]=p.value;let a=parseInt(s.uncovered_lines||"0",10),c=parseFloat(s.coverage||"0");(a>0||c<80)&&o.push({id:r.key,ruleKey:"coverage:uncovered_lines",message:`${a} uncovered lines (${c.toFixed(0)}% coverage)`,component:r.key,filePath:r.path||this.extractFilePath(r.key),type:"COVERAGE",severity:c<50?"CRITICAL":"MAJOR",status:"UNCOVERED",effort:`${a} lines`,tags:["test-coverage","unit-test"],creationDate:new Date().toISOString()})}return o}async getDuplicationFiles(e){let t=`${this.serverUrl}/api/measures/component_tree?component=${encodeURIComponent(e)}&metricKeys=duplicated_lines_density,duplicated_blocks&qualifiers=FIL&strategy=leaves&s=metric&metricSort=duplicated_lines_density&asc=false&ps=100`,i=await this.authenticatedFetch(t);if(!i.ok)throw new Error(`Failed to fetch duplication files: HTTP ${i.status} ${i.statusText}`);let n=await i.json(),o=[];for(let r of n.components||[]){let s={};for(let p of r.measures||[])s[p.metric]=p.value;let a=parseFloat(s.duplicated_lines_density||"0"),c=parseInt(s.duplicated_blocks||"0",10);(a>0||c>0)&&o.push({id:r.key,ruleKey:"duplications:duplicated_code",message:`${a.toFixed(1)}% duplicated lines (${c} duplicated blocks)`,component:r.key,filePath:r.path||this.extractFilePath(r.key),type:"DUPLICATION",severity:a>20?"CRITICAL":"MAJOR",status:"DUPLICATED",effort:`${c} blocks`,tags:["code-duplication","refactoring"],creationDate:new Date().toISOString()})}return o}};var I=w(require("node:path")),B=w(require("node:fs/promises")),h=w(require("vscode")),k=class{workspaceRoot;fileExistsFn;findFilesFn;constructor(e){this.workspaceRoot=e?.workspaceRoot??h.workspace.workspaceFolders?.[0]?.uri.fsPath,this.fileExistsFn=e?.fileExistsFn??(async t=>{try{return await B.stat(t),!0}catch{return!1}}),this.findFilesFn=e?.findFilesFn??(async t=>(await h.workspace.findFiles(t,"**/node_modules/**",5)).map(n=>n.fsPath))}async resolveFilePath(e){if(!this.workspaceRoot)return null;let t=I.isAbsolute(e)?e:I.join(this.workspaceRoot,e);if(await this.fileExistsFn(t))return t;let i=I.basename(e);if(i){let n=await this.findFilesFn(`**/${i}`);if(n&&n.length>0)return n[0]}return null}async openFileAtLine(e,t){let i=await this.resolveFilePath(e);if(!i)return h.window.showWarningMessage(`Could not find file locally: ${e}`),!1;try{let n=h.Uri.file(i),o=await h.workspace.openTextDocument(n),r=await h.window.showTextDocument(o,{preview:!1});if(t!==void 0&&t>0){let s=t-1,a=new h.Position(s,0);r.selection=new h.Selection(a,a),r.revealRange(new h.Range(a,a),h.TextEditorRevealType.InCenter)}return!0}catch(n){return h.window.showErrorMessage(`Failed to open file: ${n.message||String(n)}`),!1}}};var R=w(require("node:path")),U=w(require("node:fs/promises")),v=w(require("vscode"));var S=class{ruleCache=new Map;fileNavigator;fetchRuleFn;readCodeSnippetFn;isExtensionInstalledFn;isAntigravityEnvFn;executeCommandFn;sendToAgentPanelFn;constructor(e){this.fileNavigator=e?.fileNavigator??new k,this.fetchRuleFn=e?.fetchRuleFn,this.readCodeSnippetFn=e?.readCodeSnippetFn,this.executeCommandFn=e?.executeCommandFn??((t,...i)=>v.commands.executeCommand(t,...i)),this.sendToAgentPanelFn=e?.sendToAgentPanelFn??(t=>{let i=v.antigravityExtensibility;return i&&typeof i.sendToAgentPanel=="function"?i.sendToAgentPanel(t):Promise.reject(new Error("antigravityExtensibility.sendToAgentPanel not available"))}),this.isExtensionInstalledFn=e?.isExtensionInstalledFn??(t=>{try{return!!v.extensions.getExtension(t)}catch{return!1}}),this.isAntigravityEnvFn=e?.isAntigravityEnvFn??(()=>{try{return!!((v.env.appName||"").toLowerCase().includes("antigravity")||process.env.GEMINI_CLI||process.env.ANTIGRAVITY_IDE||process.env.ANTIGRAVITY_AGENT)}catch{return!1}})}getAvailableAgents(){let e=[];return(this.isExtensionInstalledFn("github.copilot")||this.isExtensionInstalledFn("github.copilot-chat"))&&e.push({id:"copilot",name:"GitHub Copilot",description:"VS Code Copilot Chat",focusCommand:"workbench.action.chat.open"}),(this.isAntigravityEnvFn()||this.isExtensionInstalledFn("google.antigravity")||this.isExtensionInstalledFn("google.gemini"))&&e.push({id:"antigravity",name:"Antigravity Agent",description:"DeepMind Antigravity Agent",focusCommand:"antigravity.openChatView"}),this.isExtensionInstalledFn("anthropic.claude-code")&&e.push({id:"claude-code",name:"Claude Code",description:"Anthropic Claude Code for VS Code",focusCommand:"workbench.view.extension.claude-sidebar"}),this.isExtensionInstalledFn("saoudrizwan.claude-dev")&&e.push({id:"cline",name:"Cline",description:"Autonomous AI coding agent",focusCommand:"claude-dev.focus"}),this.isExtensionInstalledFn("rooveterinaryinc.roo-cline")&&e.push({id:"roo-code",name:"Roo Code",description:"Roo Code coding agent",focusCommand:"roo-cline.focus"}),this.isExtensionInstalledFn("continue.continue")&&e.push({id:"continue",name:"Continue",description:"Continue open-source AI assistant",focusCommand:"continue.focusContinueInputView"}),e.push({id:"clipboard",name:"Clipboard Only",description:"Copy prompt to clipboard"}),e}async getRule(e){if(this.ruleCache.has(e))return this.ruleCache.get(e);let t;return this.fetchRuleFn?t=await this.fetchRuleFn(e):t={key:e,name:e,cleanDesc:"Adhere to SonarQube quality standard for this rule."},this.ruleCache.set(e,t),t}detectLanguage(e){switch(R.extname(e).toLowerCase()){case".ts":case".tsx":return"typescript";case".js":case".jsx":return"javascript";case".vue":return"vue";case".html":return"html";case".css":return"css";case".py":return"python";case".java":return"java";case".go":return"go";case".rs":return"rust";default:return""}}async readCodeSnippet(e,t){if(this.readCodeSnippetFn)return this.readCodeSnippetFn(e,t);let i=await this.fileNavigator.resolveFilePath(e);if(!i)return null;try{let o=(await U.readFile(i,"utf-8")).split(/\r?\n/),r=o.length,s=t!==void 0&&t>0?t:1,a=Math.max(1,s-10),c=Math.min(r,s+10),p=[];for(let u=a;u<=c;u++){let g=o[u-1],b=u===s?" ---> [ISSUE HERE] ":"      ";p.push(`${u.toString().padStart(4," ")} |${b}${g}`)}return{snippet:p.join(`
...
`,t}async dispatch(e,t,i,n){try{await v.env.clipboard.writeText(e)}catch{}if(i&&i.line&&await this.fileNavigator.openFileAtLine(i.filePath,i.line),t==="copilot")try{return await this.executeCommandFn("workbench.action.chat.open",{query:e}),v.window.showInformationMessage("Dispatched Fix Prompt to GitHub Copilot Chat!"),{ok:!0,message:"Dispatched to GitHub Copilot Chat."}}catch{return v.window.showInformationMessage("Prompt copied to clipboard! Paste it into GitHub Copilot Chat."),{ok:!0,message:"Copied to clipboard (Copilot chat command not found)."}}if(t==="antigravity"){try{let s=[],a=n&&n.length>0?n:i?[i]:[],c=new Set;for(let p of a)if(p.filePath){let u=await this.fileNavigator.resolveFilePath(p.filePath);if(u&&!c.has(u)){c.add(u);let g=p.line&&p.line>0?p.line-1:0;s.push({uri:v.Uri.file(u),startLine:g,endLine:g})}}return await this.sendToAgentPanelFn({message:e,files:s.length>0?s:void 0,autoSend:!1}),v.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}for(let s of["workbench.action.chat.open","antigravity.prioritized.chat.open","workbench.action.openChat"])try{return await this.executeCommandFn(s,{query:e}),v.window.showInformationMessage("Dispatched Fix Prompt to Antigravity Chat!"),{ok:!0,message:"Dispatched to Antigravity Chat."}}catch{}try{return await this.executeCommandFn("antigravity.openChatView"),v.window.showInformationMessage("Antigravity Chat opened & prompt copied to clipboard! Press Cmd+V / Ctrl+V to paste."),{ok:!0,message:"Chat opened and prompt ready in clipboard."}}catch{return v.window.showInformationMessage("Fix Prompt copied to clipboard for Antigravity Agent! Paste it into your agent chat."),{ok:!0,message:"Prompt ready in clipboard for Antigravity Agent."}}}let o=this.getAvailableAgents().find(s=>s.id===t),r=o?.name||(t==="claude-code"?"Claude Code":t==="cline"?"Cline":t==="roo-code"?"Roo Code":t==="continue"?"Continue":"Clipboard");if(t==="claude-code"){let s=["workbench.view.extension.claude-sidebar","claude-code.focus","claude.focus"];for(let a of s)try{await this.executeCommandFn(a);break}catch{}}else if(o?.focusCommand)try{await this.executeCommandFn(o.focusCommand)}catch{}return t==="clipboard"?(v.window.showInformationMessage("Fix Prompt copied to clipboard!"),{ok:!0,message:"Prompt ready in clipboard."}):(v.window.showInformationMessage(`Fix Prompt copied to clipboard for ${r}! Paste it into your agent chat.`),{ok:!0,message:`Prompt ready in clipboard for ${r}.`})}};var M=w(require("vscode")),m=class{static channel;static initialize(e){return e?this.channel=e:this.channel||(this.channel=M.window.createOutputChannel("Sonar Agent",{log:!0})),this.channel}static sanitize(e){if(!e)return"";let t=e.replace(/https?:\/\/[^\s"'`<>]+/gi,"[SERVER]");return t=t.replace(/(token|bearer|authorization|password)\s*[:=]\s*[^\s,;&]+/gi,"$1: [REDACTED]"),t}static info(e){this.channel||this.initialize(),this.channel?.info(this.sanitize(e))}static warn(e){this.channel||this.initialize(),this.channel?.warn(this.sanitize(e))}static error(e,t){this.channel||this.initialize();let i="";t&&(t instanceof Error?i=` - ${t.message}`:i=` - ${String(t)}`),this.channel?.error(this.sanitize(`${e}${i}`))}static debug(e){this.channel||this.initialize(),this.channel?.debug(this.sanitize(e))}static show(){this.channel?.show(!0)}static dispose(){this.channel?.dispose(),this.channel=void 0}};var P=class{constructor(e,t,i,n,o){this.extensionUri=e;this.projectDetector=t;this.fileNavigator=i??new k,this.agentDispatcher=n??new S({fileNavigator:this.fileNavigator}),this.diagnosticCollection=o??l.languages.createDiagnosticCollection("SonarQube")}static viewType="sonarAgent.overviewView";_view;fileNavigator;agentDispatcher;diagnosticCollection;resolveWebviewView(e,t,i){this._view=e,e.webview.options={enableScripts:!0,localResourceRoots:[this.extensionUri]},e.webview.html=this._getHtmlForWebview(e.webview),e.webview.onDidReceiveMessage(async n=>{try{switch(n.command){case"init":{await this._syncState();break}case"connect":{await this._handleConnect(n.serverUrl,n.token);break}case"disconnect":{await this._handleDisconnect();break}case"selectProject":{n.projectKey&&(await this.projectDetector.setProjectKey(n.projectKey),await this._syncState());break}case"openProjectPicker":{await this.promptProjectSelection();break}case"fetchDetails":{await this._handleFetchDetails(n.category);break}case"openFile":{await this.fileNavigator.openFileAtLine(n.filePath,n.line);break}case"sendToAgent":{await this._handleSendToAgent(n.item,n.targetAgentId);break}case"sendBatchToAgent":{await this._handleSendBatchToAgent(n.items,n.targetAgentId);break}case"setTargetAgent":{await l.workspace.getConfiguration("sonarAgent").update("defaultAgent",n.agentId,!0);break}case"refresh":{await this._syncState();break}}}catch(o){console.error("[SonarAgent] Webview message handling error:",o),this._view?.webview.postMessage({type:"error",message:o?.message||"Error processing request."})}}),this._syncState(),e.onDidChangeVisibility(()=>{e.visible&&this._syncState()})}async refresh(){this._view&&await this._syncState()}async promptProjectSelection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!e.serverUrl||!t){l.window.showWarningMessage("Please connect to SonarQube first.");return}let n=await new x({serverUrl:e.serverUrl,token:t}).fetchProjects(),o={label:"$(edit) Enter Project Key manually...",description:"Type the exact project key from SonarQube",detail:"Use this if your project is not listed or search is restricted"},r=[o,...n.map(a=>({label:a.name,description:a.key,detail:a.key===e.projectKey?"(Currently selected)":void 0}))],s=await l.window.showQuickPick(r,{placeHolder:"Select a SonarQube project or enter key manually",matchOnDescription:!0});if(s===o){let a=await l.window.showInputBox({prompt:"Enter the SonarQube Project Key",placeHolder:"e.g. org.company:my-project",value:e.projectKey||"",validateInput:c=>c.trim()?null:"Project Key cannot be empty"});if(a&&a.trim()){let c=a.trim();await this.projectDetector.setProjectKey(c),await this._syncState(),l.window.showInformationMessage(`Active SonarQube project set to: ${c}`)}return}s&&s.description&&(await this.projectDetector.setProjectKey(s.description),await this._syncState(),l.window.showInformationMessage(`Active SonarQube project set to: ${s.label}`))}async promptConfigureConnection(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),i=!!(e.serverUrl&&t),n=[{label:"$(link) Update Server URL & Token",description:i?"Connected":"Not connected",detail:e.serverUrl?`Server: ${e.serverUrl}`:"Configure SonarQube host URL and authentication token",action:"updateCredentials"},{label:"$(folder-active) Select Sonar Project",description:e.projectKey?"Active":"Not selected",detail:e.projectKey?`Current project: ${e.projectKey}`:"Choose an active project on the server",action:"selectProject"},{label:"$(gear) Open Extension Settings",detail:"Configure default AI agent and advanced preferences",action:"openSettings"},{label:"$(output) Show Extension Logs",detail:"Open the Sonar Agent output log channel",action:"showLogs"}];i&&n.push({label:"$(debug-disconnect) Disconnect & Reset Credentials",detail:"Remove stored token from OS Keychain and disconnect",action:"disconnect"});let o=await l.window.showQuickPick(n,{placeHolder:"Sonar Agent: Configure Connection & Settings",matchOnDescription:!0,matchOnDetail:!0});if(o)switch(o.action){case"updateCredentials":await this.promptUpdateCredentials(e.serverUrl);break;case"selectProject":await this.promptProjectSelection();break;case"openSettings":await l.commands.executeCommand("workbench.action.openSettings","@ext:chulit.sonar-agent");break;case"showLogs":m.show();break;case"disconnect":await l.commands.executeCommand("sonarAgent.resetConnection");break}}async promptUpdateCredentials(e,t){let i=e||(await this.projectDetector.getConfig()).serverUrl||"http://localhost:9000",n=t||"";for(;;){let o=await l.window.showInputBox({title:"SonarQube Connection (1/2)",prompt:"Enter the SonarQube Server URL",placeHolder:"http://localhost:9000 or https://sonar.example.com",value:i,ignoreFocusOut:!0,validateInput:c=>{let p=c.trim();if(!p)return"Server URL is required";if(!p.startsWith("http://")&&!p.startsWith("https://"))return"Server URL must start with http:// or https://";try{if(!new URL(p).hostname)return"Please enter a valid URL with hostname"}catch{return"Please enter a valid URL"}return null}});if(o===void 0)return;i=o.trim();let r=await l.window.showInputBox({title:"SonarQube Connection (2/2)",prompt:"Enter your SonarQube User Token",placeHolder:"sqp_...",value:n,password:!0,ignoreFocusOut:!0,validateInput:c=>c.trim()?null:"User Token is required"});if(r===void 0)return;n=r.trim();let s={ok:!1};if(await l.window.withProgress({location:l.ProgressLocation.Notification,title:"Verifying SonarQube connection...",cancellable:!1},async()=>{s=await new x({serverUrl:i,token:n}).verifyConnection()}),!s.ok){if(await l.window.showErrorMessage(`SonarQube connection verification failed: ${s.message||"Unknown error"}`,"Retry","Cancel")==="Retry")continue;return}await this.projectDetector.setServerUrl(i),await this.projectDetector.setToken(n),l.window.showInformationMessage("SonarQube connection successfully verified and saved!"),(await this.projectDetector.getConfig()).projectKey||await this.promptProjectSelection(),await this.refresh();return}}async _handleSendToAgent(e,t){let i=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),o=t||l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),r;i.serverUrl&&n&&(r=new x({serverUrl:i.serverUrl,token:n}));let s=new S({fileNavigator:this.fileNavigator,fetchRuleFn:r?c=>r.getEnrichedRule(c):void 0}),a=await s.assemblePrompt(e);await s.dispatch(a,o,e)}async _handleSendBatchToAgent(e,t){if(!e||e.length===0)return;let i=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),o=t||l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),r;i.serverUrl&&n&&(r=new x({serverUrl:i.serverUrl,token:n}));let s=new S({fileNavigator:this.fileNavigator,fetchRuleFn:r?c=>r.getEnrichedRule(c):void 0}),a=await s.assembleBatchPrompt(e);await s.dispatch(a,o,e[0],e)}async _handleFetchDetails(e){if(!this._view)return;let t=await this.projectDetector.getConfig(),i=await this.projectDetector.getToken();if(!(!t.serverUrl||!t.projectKey||!i)){this._view.webview.postMessage({type:"loadingDetails",loading:!0});try{let n=new x({serverUrl:t.serverUrl,token:i}),o=[];e==="hotspots"?o=await n.getHotspots(t.projectKey):e==="coverage"?o=await n.getCoverageFiles(t.projectKey):e==="duplications"?o=await n.getDuplicationFiles(t.projectKey):e==="reliability"||e==="security"||e==="maintainability"||e==="accepted"?o=await n.getIssues(t.projectKey,e):o=await n.getIssues(t.projectKey),this._view.webview.postMessage({type:"details",category:e,items:o}),await this.syncDiagnostics(o)}catch(n){this._view.webview.postMessage({type:"detailsError",message:n.message||"Failed to load issues."})}finally{this._view.webview.postMessage({type:"loadingDetails",loading:!1})}}}async syncDiagnostics(e){this.diagnosticCollection.clear();let t=new Map;for(let i of e){if(!i.filePath)continue;let n=await this.fileNavigator.resolveFilePath(i.filePath);if(!n)continue;let o=l.Uri.file(n),r=i.line&&i.line>0?i.line-1:0,s=new l.Range(r,0,r,100),a=l.DiagnosticSeverity.Information;i.severity==="BLOCKER"||i.severity==="CRITICAL"?a=l.DiagnosticSeverity.Error:i.severity==="MAJOR"&&(a=l.DiagnosticSeverity.Warning);let c=new l.Diagnostic(s,i.message,a);c.code=i.ruleKey,c.source="SonarQube";let p=t.get(o.toString())||{uri:o,diagnostics:[]};p.diagnostics.push(c),t.set(o.toString(),p)}for(let{uri:i,diagnostics:n}of t.values())this.diagnosticCollection.set(i,n)}async _syncState(){if(this._view)try{let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken(),i=l.workspace.getConfiguration("sonarAgent").get("defaultAgent","copilot"),n=this.agentDispatcher.getAvailableAgents(),o=i;if(n.some(r=>r.id===o)||(o=n[0]?.id||"clipboard"),e.serverUrl&&e.hasToken&&t){this._view.webview.postMessage({type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:e.projectKey,detectedFromProperties:e.detectedFromProperties??!1,hasPlaintextWarning:e.hasPlaintextCredentialsWarning??!1,projects:[],defaultAgent:o,availableAgents:n}),this._view.webview.postMessage({type:"loading",loading:!0});let r=new x({serverUrl:e.serverUrl,token:t}),s=e.projectKey,[a,c]=await Promise.allSettled([r.fetchProjects(),s?r.getOverview(s):Promise.resolve(null)]),p=a.status==="fulfilled"?a.value:[];a.status==="rejected"&&console.error("[SonarAgent] fetchProjects error:",a.reason);let u=s;!u&&p.length>0&&(u=p[0].key,await this.projectDetector.setProjectKey(u));let g=null,b;if(u&&u===s)c.status==="fulfilled"?(g=c.value,g&&m.info(`Measures updated for [${u}]: ${g.security.count} vulnerabilities, ${g.reliability.count} bugs, ${g.maintainability.count} smells, ${g.coverage.percentage.toFixed(1)}% coverage.`)):(b=c.reason?.message||"Failed to fetch project measures.",m.error(`Failed to fetch project measures for [${u}]`,b));else if(u)try{g=await r.getOverview(u),g&&m.info(`Measures updated for [${u}]: ${g.security.count} vulnerabilities, ${g.reliability.count} bugs, ${g.maintainability.count} smells, ${g.coverage.percentage.toFixed(1)}% coverage.`)}catch(C){b=C.message||"Failed to fetch project measures.",m.error(`Failed to fetch project measures for [${u}]`,b)}this._view.webview.postMessage({type:"state",state:"connected",serverUrl:e.serverUrl,projectKey:u,detectedFromProperties:e.detectedFromProperties??!1,hasPlaintextWarning:e.hasPlaintextCredentialsWarning??!1,projects:p,overview:g,overviewError:b,defaultAgent:o,availableAgents:n}),this._view.webview.postMessage({type:"loading",loading:!1})}else this._view.webview.postMessage({type:"state",state:"onboarding",serverUrl:e.serverUrl||"http://localhost:9000",defaultAgent:o,availableAgents:n})}catch(e){console.error("[SonarAgent] _syncState error:",e),m.error("Failed to synchronize Sonar Agent state",e),this._view?.webview.postMessage({type:"error",message:e.message||"Failed to synchronize Sonar Agent state."}),this._view?.webview.postMessage({type:"loading",loading:!1})}}async _handleConnect(e,t){let i=(t||"").trim(),n=(e||"").trim();if(!n||!i){this._view?.webview.postMessage({type:"error",message:"Server URL and User Token are required."});return}!n.startsWith("http://")&&!n.startsWith("https://")&&(n="http://"+n),n=n.replace(/\/+$/,""),this._view?.webview.postMessage({type:"connecting"}),m.info("Verifying SonarQube credentials...");try{let r=await new x({serverUrl:n,token:i}).verifyConnection();if(!r.ok){m.warn(`Connection verification failed: ${r.message||"Unknown error"}`),this._view?.webview.postMessage({type:"error",message:r.message||"Connection verification failed."});return}await this.projectDetector.setServerUrl(n),await this.projectDetector.setToken(i),m.info("SonarQube connection verified and saved."),l.window.showInformationMessage("SonarQube connection successfully verified!"),await this._syncState()}catch(o){console.error("[SonarAgent] _handleConnect error:",o),m.error("Unexpected connection error occurred",o),this._view?.webview.postMessage({type:"error",message:o?.message||"Unexpected connection error occurred."})}}async _handleDisconnect(){let e=await this.projectDetector.getConfig(),t=await this.projectDetector.getToken();if(!!!(e.serverUrl&&t)){this._view?.webview.postMessage({type:"disconnected",message:"Connection credentials cleared."}),l.window.showInformationMessage("Sonar Agent connection inputs cleared.");return}await l.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await this.projectDetector.deleteToken(),m.info("SonarQube credentials removed and disconnected."),this._view?.webview.postMessage({type:"disconnected",message:"Disconnected from SonarQube. Credentials removed."}),l.window.showInformationMessage("SonarQube credentials have been removed."),await this._syncState())}_getHtmlForWebview(e){let t=V();return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
</html>`}};function V(){let d="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)d+=e.charAt(Math.floor(Math.random()*e.length));return d}var y=w(require("vscode"));var A=class{static providedCodeActionKinds=[y.CodeActionKind.QuickFix];projectDetector;fileNavigator;customDispatcher;constructor(e){this.projectDetector=e.projectDetector,this.fileNavigator=e.fileNavigator??new k,this.customDispatcher=e.dispatcher}provideCodeActions(e,t,i,n){let o=[];for(let r of i.diagnostics)if(r.source&&r.source.toLowerCase().includes("sonar")){let s=r.code?`\u26A1 Send to AI Agent (${r.code})`:"\u26A1 Send to AI Agent (SonarQube)",a=new y.CodeAction(s,y.CodeActionKind.QuickFix);a.command={command:"sonarAgent.fixWithAgent",title:"Send to Agent",arguments:[r,e]},a.diagnostics=[r],a.isPreferred=!0,o.push(a)}return o}async executeFixWithAgent(e,t){let i=await this.projectDetector.getConfig(),n=await this.projectDetector.getToken(),o;i.serverUrl&&n&&(o=new x({serverUrl:i.serverUrl,token:n}));let r=this.customDispatcher??new S({fileNavigator:this.fileNavigator,fetchRuleFn:o?C=>o.getEnrichedRule(C):void 0}),s=await r.getAvailableAgents(),c=y.workspace.getConfiguration("sonarAgent").get("defaultAgent");(!c||!s.some(C=>C.id===c))&&(c=s[0]?.id||"antigravity");let p="MAJOR";e.severity===y.DiagnosticSeverity.Error?p="CRITICAL":e.severity===y.DiagnosticSeverity.Information?p="MINOR":e.severity===y.DiagnosticSeverity.Hint&&(p="INFO");let u=y.workspace.asRelativePath?y.workspace.asRelativePath(t.uri):t.fileName,g={id:String(e.code||"sonar-issue"),ruleKey:String(e.code||""),message:e.message,component:u,filePath:u,line:e.range.start.line+1,severity:p,type:"CODE_SMELL",status:"OPEN",tags:[],creationDate:new Date().toISOString()},b=await r.assemblePrompt(g);return r.dispatch(b,c,g)}};function W(d){let e=m.initialize();d.subscriptions.push(e),m.info("Sonar Agent extension activated.");let t=f.workspace.workspaceFolders?.[0]?.uri.fsPath,i=new j({secretStorage:d.secrets,workspaceConfig:{get:(r,s)=>f.workspace.getConfiguration("sonarAgent").get(r,s),update:(r,s,a)=>f.workspace.getConfiguration("sonarAgent").update(r,s,a)},workspaceRoot:t}),n=new P(d.extensionUri,i);d.subscriptions.push(f.window.registerWebviewViewProvider(P.viewType,n)),d.subscriptions.push(f.commands.registerCommand("sonarAgent.refresh",async()=>{await n.refresh()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.configure",async()=>{await n.promptConfigureConnection()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.selectProject",async()=>{await n.promptProjectSelection()})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.resetConnection",async()=>{await f.window.showWarningMessage("Are you sure you want to disconnect and remove stored SonarQube credentials?",{modal:!0},"Disconnect")==="Disconnect"&&(await i.deleteToken(),await n.refresh(),f.window.showInformationMessage("SonarQube credentials have been removed."))})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.showLogs",()=>{m.show()}));let o=new A({projectDetector:i});d.subscriptions.push(f.languages.registerCodeActionsProvider({scheme:"file"},o,{providedCodeActionKinds:A.providedCodeActionKinds})),d.subscriptions.push(f.commands.registerCommand("sonarAgent.fixWithAgent",async(r,s)=>{!r||!s||await o.executeFixWithAgent(r,s)}))}function Q(){m.dispose()}0&&(module.exports={activate,deactivate});
