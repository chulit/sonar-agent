...
- [x] Create `SonarLocalScanner` with the following public interface:
...
- [x] Mapping from `vscode.Diagnostic` to `SonarDetailItem`: `ruleKey` from `diagnostic.code`, `line` = `range.start.line + 1`, `severity` from `DiagnosticSeverity`, `filePath` as workspace-relative path, `source = 'sonarlint'`.
- [x] Unit tests for `isSonarLintInstalled()`, `getLocalDiagnostics()` filtering, diagnostic mapping correctness, and `runCliScan()` binary resolution using injected mocks.
